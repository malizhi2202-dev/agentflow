...
- **当前阶段**: 2-design **出口完成** —— `DESIGN.md` ＋ 3 份 ADR（`.specs/adr/001~003`）落盘：交付物形态锁定「零依赖单文件 HTML」，接口＝**标记契约 + K 系列（K1…K11） 派生对账闸**（§1.5），15 条决策各带备选/理由/代价，10 条风险含缓解，§9 沉淀 5 小节。**🛡️ G2 方案门 3/4**：🟫 ✅ `01a0c851`（两项非阻断已收：§8 票面格式 + D15 五条）／🟦 ✅ `01a0c853`（两陷阱立为 D16/D17）／🔴 ❌ 条件票 `01a0c855` **三条已全部落地并复跑**（B-1 D3 附录改「S1-S10 边界句所在域索引（只放 `id`）」+ 新闸 **K11** + UAT-8；B-2 `.html` 推论降级——只避开 `artifact.py` 一层，`admin_api.py:126-135` 是无鉴权任意文件读，实测 `os.path.join` 丢前缀，D11/R5/§9.5/ADR-001 同步改口径并登记第 13 条议题 `admin-file-read-jail`；B-3 新增 **§7.6 复现配方**，十条断言实跑全中、样本落 `$TMPDIR/proto-samples/` 禁入库），待其复验改票；🟩 领域专家未到。**R13 纪律**：票未集齐即静默结束 turn，不催票、不进 2a、不写交付稿；🔴 三条依 R9.2 需另一角色二次确认，终裁时指定 🟫 Master 核。
- **当前 Task**: —
- **中断任务**（R1.5 清窗）: 无（G2 待票 3/4 期间停在 2-design 出口；🔴 条件票三条已落地，等改票 + 🟩 第 4 票）
- **会话开始建议**: 先数 G2 票（现 3/4：🟩 领域专家未到、🔴 待复验改票；集齐即按 R13.2 终裁，🔴 三条依 R9.2 需另一角色二次确认，终裁时指定 🟫 Master 核）（未集齐 → 静默待票，不推进）。票齐后进 2a-ui-design：读 `@.specs/product-prototype-refresh/DESIGN.md` §1.5（标记契约＝接口名，改字面串必须同笔改 `REQUIREMENT.md` 该条 AC 命令）＋ §0/§6（两轴标签不可合并、交付稿不进 S1-S10 整表、`.html` 后缀必须保持）＋ `@.specs/adr/001~003`；**2a 开工前必须拿到人工对 `product-design.html` ①/② 的拍板**（现按 ② 新建起草）。需代码议题 **12 → 13 条**（本轮新登记 `admin-file-read-jail`，待需求侧并入 RESEARCH §2；在此之前交付稿 `data-slug` 仍只挂 12 条，否则 K4 假红），全部仍**不在本 change 内实现**（R7.1）。2a 只需读 DESIGN §1.5 + §8 + §7.6（配方），不必读 §7.5 的私有样本。
...
| 需求侧三处归口（🔴🟫🟦 已各自核过） | 纠正（工件归口，不由 2-design 改） | ① `BASELINE-code-facts.md:35`「`Detail` 含 6 个 tab 组件」→ 实测 **5**；② BASELINE §4 补 **S11** 边界句（`admin_api.py:126-135` 无鉴权任意文件读；依 D11 内部件持有、禁进交付稿）；③ 需代码议题 **12 → 13**：并入 `admin-file-read-jail` 后同步 `RESEARCH-competitors.md` §2 与三条计数不变式——在此之前交付稿 `data-slug` 仍只挂 12 条，否则 K4 假红 | 需求分析 | 2026-09-22 |
