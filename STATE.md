...
| 出口条件 | **5-test 测试门（4 专家投票）本轮仍不召集** —— 召集条件 = 定向 28 条全绿（现 **18 红**，17→16 因 `T-FIX-04`、+2 因本阶段新增归一化判据）+ 开放项 O-1（性能预算）/O-2（跨浏览器）/O-3（可观测）/**O-10（`T-FIX-02` 的 R4.6 四选一）**/**O-11（admin 旁路三出路，与 A07 fail-open 耦合）**有结论（TEST.md 文末表）。9 项 🔴 现结 2 项（F1 = `T-FIX-00` 本工件 · F4 = `T-FIX-04`）余 7 项。⚠️ **2.0 那道「缺 TEST.md」的门已解**，现在拦集成的是 **R2.5（7 项 🔴 未修）+ 第 1 轮未全绿 + 测试门未召集**三条，口径仍是**不集成、不合并** ‖ **进 7-integration 前必须结清清单（v10.2 由安全审计师 ①④ 点挂，主审落）**：A07 fail-open · admin 旁路口径（O-11）· **凭据使用面三条并列（F23①chat / ⑪探针链 / ⑫genai 全局 Key）→ 另开 CHANGE 优先修，与本 change 尾批人签同挂**；A10 探针链细节锚在 `T-FIX-12` ⑪ 与 REVIEW v10.2（议题包 ≠ 处置出口 —— STATE 第 5 条） |
| 待人工裁定 | **再加 1 条（O-14，来自 `T-FIX-13` 落地后的实测代价）**：收口后前端扩容出的副本一律 `not_set`（`domains.ts` 从不发 `api_key`）→ (α) 前端加「自备 key」输入（与 `T-FIX-11` 的 (a)/(b) 同族）/ (β) 人工签「允许继承模板自己的 Key」（收口后模板恒归调用者，改 `domain_api.py:248` 一行 + 4-dev 那条新断言）。**不答 = 弹性缩放功能停用**。 ‖ **新增 2 条**：① `T-FIX-04` 的「默认不带 admin 旁路」与 `T-FIX-00` 护栏 `fr2_contract_shape_and_dedup` 互斥（改代码 vs 改用例，顺序不能反，详见 `T-FIX-04-SUMMARY.md` 决策①）；② `T-FIX-02` 命中 R4.6（`_extract_capabilities` 定义 11 行 + `domain_api.py:9` 跨模块 import + `:175/:211/:285` 三处调用），四选一未答即不动手（引用图已备好）。原口径不变：**8 条 + 1 条偏好决策**（`T-FIX-13` 要不要保留 admin 旁路；安全主张不保留，主审附议但按 R18 ④ 交人签）。**注：动手修 🔴 不需要等签字，只有想跳过某个 🔴 才必须先签** —— 所以 5-test / 4-dev 现在就能动，不等人。原 8 条见 REVIEW.md（v4 新增第 6「默认域双指须 1-requirement 定口径」、第 7「`未分类` 保留字/`__none__` + FR1×FR5 无 AC」、第 8「路由/扩容规则无需求溯源」）。**第 1 条已加限定**：只有 F6 的「字号」那一半可进「已知接受」，间距/圆角属机械替换不得换签字；签审查口径须连带确认 `CHANGE.md` 把测试放在 `.specs/` 这个错路径。**第 5 条**：`:208` 凭据链**不得缓办**，只能选「本 change 内修」或「另开 CHANGE 优先修」 ‖ **v10.2 更新（详细以 REVIEW v10.1/v10.2 账本为准，人批时读账本不读此行摘要）**：O-14 已改三分支 **(α)/(β)/(γ)** 且 (β) 前置 = A07 fail-closed、F26（PUT 空串 Key 撤销 no-op，`agents_api.py:112`）并入 (β) 代价；O-11 补**第四类旁路**（无头即 admin；`test_missing_user_header` 今天即红 403 钉着 200 —— A07 与它必须同批改，修完 3 豁免站点 ≠ 收口）；凭据使用面三条（chat/探针/genai 全局 Key）按第 5 条口径走**另开 CHANGE**，议题包只留锚点；新增 **O-15**（`desired_replicas` 约束「域」还是「调用者份额」） |
| **7-integration 入口核验（2026-09-24 10:2x · 集成归档）** | **闸门 = 关闭并保持**。G4 4/4 结的是「审查工件可交接」，本行核的是「可集成」，两者距离已逐条量在 `.specs/capability-groups/UAT.md` 与 `.specs/ALIGNMENT.md`（R15.1 第 1 轮；本仓此前**从无 ALIGNMENT 件**）。**一手复跑 @ tip `0e8ee534`**：定向 `16F/12P/1S`（29 条）· 全量 `57/146/7` · 非 capability 红按文件数 **28+6+4+3=41** · `tsc=32`（=32 护栏守住，本 change 零新增类型债）· `npm run build` **必红**（tsc 阶段即失败 → 无生产构建）· vitest `66/6 files`、`capability 14`。**R2.5：9 🔴 = 3 已修（F1/F4/F16）+ 6 未修**（F2 F3 F5 F6 F7 F10；取值逐个重取：LIKE 仍 `agents_api.py:53`、`get("capabilities"` 仍 **7** 处且 `capability_service.py` 不存在、`#fff`=6、`var(--s/--r-`=0、`a.health`=2）·**「已知接受」签字 0 份**（MALIZHI-14 全 12 条评论 author_type 皆 agent，member 0 条）。**上面「出口条件」行点挂的集成前清单 7 项 0 项结清**：A07 `main.py:204-210`+`auth.py:181-186` · O-11 `_filter_owner:20-23`（含第四类）· 使用面 chat `chat_api.py:18`→`chat_service.py:48/:103` / 探针 `agent_probe_service.py:207/:223`+`control_plane_api.py:69` / genai `llm_providers.py:235/:256` · 无门枚举 `gateway_api.py:39-49` · F26 `agents_api.py:112`。**UAT 0 条执行**（6 条脚本已登记待人工，含 F10「顶部健康卡是否为 0」—— 不为 0 则重开 F10）。**ALIGNMENT 判 D2/D5 四项未裁 → kit-7 §0 独立禁集成**（O-11×A07 · O-14 三分支 · O-10 · FR5 两桶+溯源）。**动作边界**：不合并（R15.2）、不归档、不改他人单写者文件（TEST.md 三处过时记进 ALIGNMENT D1-2/D1-3 并附我复算的正确取值，不代刷）。下一轮 S-align 基线 = `0e8ee534` + 本核验提交 |
...
**change `capability-groups` · 7-integration 入口核验已完成，闸门关闭中**：G4 4/4 已结（6-review 出口）→ 集成前核验 = **R2.5 六项 🔴 未修 + 人工签字 0 份 + `STATE` 集成前清单 7 项 0 项结清 + UAT 0 条执行**（全部证据与命令见 `.specs/capability-groups/UAT.md`；五层对齐第 1 轮见 `.specs/ALIGNMENT.md`，其 D2/D5 四项未裁 = 独立禁集成条款）。等的是**人工批单（O-9/O-10/O-11/O-14/O-15 + 待裁定 1–8）+ 修复批（`T-FIX-14` 在飞 → 02→01 → F5/F6/F7/F10 面）**，不是「要不要继续」。

> 前值保留（本节 2026-09-24 之前一直停在 discovery 口径，与本文件顶部状态行互斥 → 记为 ALIGNMENT 漂移 **D4-3**，本轮按 A1 合并）：**产品化调研与文档重写**（code-kit discovery 第 1 轮 + P-product 产物重写）
