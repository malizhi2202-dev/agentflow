...
**触摸目标实测**：左栏项 `13px/1.5` 行高 + `padding:7px 10px`（逐字同形，见 §6.1 例外集）≈ **35.5px** 高 → WCAG 2.5.8（24px, AA）✅；2.5.5（44px, AAA）不达 → 登记 §14-⑩，不为 AAA 破坏与产品侧栏同宽的几何。徽标 chip 非控件不受该条约束。
...
# (17) 打印不承诺必须显式声明（🟦 非阻断3）
grep -c '不支持打印' "$F"
# (18)(19) A 级两件套：lang 与 文档 title（🔴 缺口 1/2；期望 1 与 >=1）
grep -c '<html[^>]*lang=' "$F"
grep -c '<title>[^<]' "$F"
# (20) 每枚徽标都有可读文案（🔴 缺口 4 的等价式：短词已改文本节点，空 chip 即缺陷；期望 0）
grep -oE '<span class="chip[^>]*>[^<]*</span>' "$F" | grep -cE '>[[:space:]]*</span>'
# (21) 状态与图形的描边禁低对比档（🔴 缺口 5；期望 0）
grep -cE '(border-color|stroke): *var\(--(border|border-strong|blue-border)\)' "$F"
...
| U17 打印声明 | ✅ 1 | U18 `<html lang>` | ✅ 1 | U19 文档 `<title>` | ✅ 2（文档 + SVG 各一） |
| U20 空 chip | ✅ 0 | U21 低对比描边 | ✅ 0 | U22 表格 `caption` | ✅ PASS（2==2） |
...
| ⑩ | **两处 AA/AAA 缺口是有理由的接受，不是遗漏**：(i) 1.4.10 Reflow —— 200% 缩放（1280→640 CSS px）下固定 `200px+1000px` 版心会出横向滚动，NFR 的「不承诺移动端」是设备豁免不是缩放豁免；(ii) 2.5.5 触摸目标 AAA —— 左栏项实测 ≈35.5px，2.5.8(AA/24px) 已过、AAA(44px) 不达 | 修法都要付更大代价：断点需第二套版心宽度（破 §6.2 白名单与「与产品侧栏同宽」），44px 目标高会破坏左栏密度。已在 §6.2 写成显式已知例外 | 需求分析（要不要为缩放承诺一套响应式版心）+ 资深用户体验官（44px 一档是否纳入本产品的 AA+ 基线） |
...
| `T-UI-04` | C1/C2/C3 + C13 左栏项（13 条 `href="#pg-*"` 按 N8 现算；可见 `💬` 包 `aria-hidden`）+ `:target` 用 `--blue` outline + 域章属性顺序（`class` 前 `id` 后）+ 域章内顺序钉法 + 页集覆盖 | `grep -c 'class="domain"' → 12`（K6c）+ U14 0 + U15 0 + U15b 0 + U16 空 + U18 1 + U19 ≥1 + U24 ≥1 + **U25/U27 无输出与 0 + U2a PASS** + U5a/U5b PASS + **K1 用稳健式**（先 `sed -E 's/（[^（）]*）//g'` 剥全角括号注解、再 `tr '、' '\n'`）→ `comm` 双向空；用现式会得 33 项假红，见 §10-D |
...
| `T-UI-08` | 可达性收尾 + C14 打印声明行：skip-link（**负号 `left:-9999px`**，U23/U23b）/ `:focus-visible` / `nav[aria-label]` / `caption+th[scope]` / reduced-motion 兜底 / `<details>` | U7 0 + U8 空 + U9 ≥1 |
...
- [x] **门禁**：G2a **3/4 通过** —— 🟫 ✅（维持，四条 🟡＋(b)(c) 落地，另给 6 处一格级残留已全收）· 🟦 ✅（首轮 ❌ 两条阻断复跑通过并改票，撤回"聚合视图"建议）· 🟩 ✅（5 条修口落地）· 🔴 ❌（五条缺口已落地待其复跑，异议全文按 R13 记入 §18）→ **本阶段出口成立，交 3-task**；四位的复算数字与本轮 75 行断言一并留档
...
🔴 无障碍专家: ❌ 对比度维逐格复算全过（21 个比值全对、1 个偏差由她纠正我 §4 的 `--border` 1.06 → 1.18）并背书；缺口集中在读屏 A 级与图形对比：`lang`/`<title>` 零命中、§9 键盘路径被 `<a data-nav>` 无 `href` 写法本身走不通、§4 自订「交互态一律 `--blue`」被 `--blue-border`/`--border-strong`（1.27–1.33）当场破掉。**五条已落地**：①C0 `lang`＋U18 ②C0 `<title>`＋U19 ③按她的出路 (a)（N8/C13/U15/U16，她的核式改稳见 §13.2）④徽标覆盖改 U20 等价式（文本节点形态下 `aria-label` 覆盖率闸会把合规稿全判红）⑤`:target` 走 `--blue`（U14）＋ `.node` 描边走 `--text-secondary`（U21）。非阻断六条：§4 数值改正、1.4.10 与 2.5.5 写成显式已知例外（§14-⑩）、`scroll-behavior` 出处与 smooth 已在上一轮改正、`border-width`/media 假红已随 §13.1-2 修口消失（clip `width:1px` 仍红→已钉唯一隐藏形态）、emoji `aria-hidden` 采纳（U24）。
结果: 3/4 通过（🟫 ✅ 🟦 ✅ 🟩 ✅ ／ 🔴 ❌）→ 按 R13 表 **3/4 = ✅ 进入下一阶段**，反对意见按规则记入工件末尾（§18），不问「要继续吗」。🔴 的五条缺口按其给定形态全部落地并机验（U14/U18/U19/U20/U21/U22/U23/U24），她说"落完即转 ✅"、复验入口在 §13 与 §13.2；她的票若最终维持 ❌ 也不改变 3/4 的裁决，只是 §18 的存档从"待复跑"变"维持异议"

---

## 17. 本门票面为什么落在这个工件里（3/4 的反对意见存档见 §18，在本节之后）
...
## 18. 3/4 票面下的反对意见存档（R13：3/4 通过必须把反对意见记入工件末尾）

**🔴 无障碍专家 ❌**（她复验到 `486453d5`，早于本轮修法）五条缺口与当前状态：① `<html lang>` 缺失（3.1.1 A 级）→ **已落地** C0 ＋ U18（实测 1）；② 文档级 `<title>` 缺失（2.4.2 A 级）→ **已落地** C0 ＋ U19（实测 2）；③ 键盘路径在 `<a data-nav>` 无 `href` 的写法下不成立（2.1.1/2.4.3/2.4.4）→ **按她的出路 (a) 落地** N8/C13/U15/U16（该点与 🟦 阻断二同源，R9.2 二次确认成立；她给的核对命令属性序敏感，改稳与复现见 §13.2）；④ `::after` 单写下漏一个 `aria-label` 会静默丢失（1.1.1/4.1.2）→ **形态已被 🟫 ④ 换成文本节点**，她的覆盖率式会把合规稿全判红，等价式 **U20**（空 chip 0）＋ **U26**（短词限两字）替代，**待她确认这条替代**；⑤ 状态与图形的非文本对比 1.27–1.33 破本文件自订的 3:1（1.4.11）→ **已落地** `:target`→`--blue`（U14）＋ `.node`→`--text-secondary`（U21）＋ §3.1 角色行 ＋ **U27** 全稿零引用。她另登记的六条非阻断：`scroll-behavior` 出处与 smooth（上一轮随 🟫 ② 已改）、`--border` 1.06→**1.18**（她复算纠正我 → §10-G）、1.4.10 Reflow 与 2.5.5 AAA 写成有理由的已知例外并归口 §14-⑩、两条形态坑（焦点环宽度只能写 `outline:`、skip-link 必须负号）已钉死并加 U23/U23b、emoji `aria-hidden` 采纳（U24）。
