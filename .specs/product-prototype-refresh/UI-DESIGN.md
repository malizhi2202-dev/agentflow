...
| 文档根（**本轮补读，初版漏了**） | `frontend/index.html` 实测：`:2` `<html lang="zh-CN" data-theme="dark">` · `:4` `<meta charset="UTF-8">` · `:6` `<title>AgentFlow</title>` —— 产品侧本来就有 A 级基线，是我的八维观察没读这个文件（🔴 缺口①② 的根因）→ §10-F | `frontend/index.html:2/4/6` |
...
| hairline | `--border` / `--border-strong` | `rgba(255,255,255,0.06/.10)` | 分隔线、表格行线 | 当"可见边框"表达交互态（实测 1.18 / 1.34，不足 3:1）；**`--blue-border` 同档禁用**（叠底 1.30–1.48，见 §6.4 `:target`） |
| 装饰蓝框 | `--blue-border` | `rgba(84,140,240,0.2)` | **本稿不使用**（🟫 #2：修法落地后全文对它只剩"禁用"，而产品侧实测 `grep -rn 'var(--blue-border)' frontend/src` → **0 引用** → 它既不是本稿用途也不是既有语汇；我原先写"本稿唯一用途：选中卡片的静态装饰边"是**虚设**，且"选中"恰是同行「绝不用于」禁止的态）→ 登记为**禁用语义用色**，并由 **U27 机验全稿零 `var()` 引用**，关掉"给非 `:target` 的选中态画一条 1px 蓝框"这条 U14 拦不到的合法绕过 | 表达任何交互态（`:target`/`hover`/`focus`）——实测叠底 **1.30:1（on `--bg-app`）/ 1.33:1（on `--bg-card`）**，与 hairline 同档；交互态一律 `--blue`（🟫 ①＋🟦 阻断一，U14 机验）。**登记理由**：本稿此前在 §6.4/C2/§9 三处用它表达 `:target`，却未在此表列行（实测该行计数 0）→ 补上，颜色权威表不再少角色 |
...
| `--border`（hairline） | `--bg-card` / `--bg-app` | **1.18 / 1.14**（🔴 复算纠正本表原值 1.06：我原先把 alpha 层的合成底算错了，方向一致但数值不对；§4 是全组复算基准表 → 已按 WCAG 相对亮度法复算对齐） | — | 只做分隔线；**不用于表达交互态**（1.4.11 需 3:1）→ 交互态一律 `--blue` |
| `--border-strong`（强框） | `--bg-card` / `--bg-input` | **1.34 / 1.27** | — | 待标注块 C6 的 1px 静框（非语义）；**不得**做 `.node` 描边（见下） | 
| `--blue-border`（装饰蓝框） | `--bg-card` / `--bg-app` / `--bg-input` | **1.33 / 1.30 / 1.29** | — | **只当装饰**；`:target`/`hover`/`focus` 一律禁用（U14） |
| `--blue`（`:target`/焦点环描边）**压在 `--bg-selected`(0.1) 叠 `--bg-card` 的底上** | 最坏情形（outline 与选中底同时生效） | **4.65** | ≥ 3:1（非文本对比下限） | ✅ 支持 C2/§9/§6.4 三处现钉形态。🔴 二轮给的数，我用同一 WCAG 相对亮度法复算：alpha 0.1 → **4.65**（我第一次误代 0.08 得 4.78，按 `tokens.css:15` 真值 0.1 才是这条） |
| `--bg-selected`（选中底） | 叠 `--bg-card` | **1.14**（ΔL +0.057） | — | `:target` 的**辅助**通道，判定不依赖它 |
| 框图节点 C12 | `.node` 填充 `--bg-card` vs 画布 `--bg-input`；描边旧值 `--border-strong` | **1.12 / 1.27** → 描边改 `--text-secondary` 后 **6.85** | ❌→✅ | 1.4.11：节点边界是"读懂框图必需"的图形对象 → 描边走 `--text-secondary`（🔴 缺口 5 第二处；配色最终裁定权在 🟫，我只钉数值下限） |
...
**间距字面量的例外集（🟫 #3：不许只靠散文守「禁表外 px」）** —— 以下 6 个字面 px **允许**出现在 `padding`/`margin`/`gap`，且**必须**带出处：`1px`·`6px`（`.badge` 内距，`tokens.css:156-157`）· `3px`（`.badge` 的 `gap:3px`，`tokens.css:155`）· `7px`·`10px`（左栏 nav 项 `padding:'7px 10px'`，`App.tsx:287/316/341`）· `2px`（nav 项 `marginBottom:2`，`App.tsx:288/317`）。它们的存在理由是**「与后台无法区分」优先于 scale 纯度**，不是 4-dev 的自由裁量区。机验（期望**无输出**）：

```bash
# U25：先排 :root，再取 padding/margin/gap 的字面 px，必须 ⊆ 8 档 ∪ 例外集
sed -n '/^:root *[{]/,/^ *}/!p' "$F" | grep -oE '(padding|margin|margin-bottom|margin-top|gap):[^;}]*' | grep -oE '[0-9]+px' | sort -u | grep -vE '^(1px|2px|3px|4px|6px|7px|8px|10px|12px|16px|20px|24px|32px|40px)$'
# U26：徽标短词限两字（🟦 #2；唯一四字例外「竞品建议」；期望无输出）
grep -oE '<span class="chip[^"]*">[^<]{3,}</span>' "$F" | grep -v 竞品建议
# U27：--blue-border 全稿零引用（🟫 #2；期望 0）
grep -c 'var(--blue-border)' "$F"
```
实测本稿补集里出现的间距字面量恰为 `1px 2px 3px 6px 7px 10px` → 三条全部**无输出/0** ✅；负例 `margin-bottom:5px` 会被 U25 报出、`<span class="chip chip-o1">本项目已有</span>` 会被 U26 报出（我把两处缺陷注入跑过，不是纸面断言）。**注**：U25 与 U12/U2b 同族，都**依赖排除式的 `:root` 窗口真的闭合** → 见 §13.3。
...
| `44em`（**可选**，非白名单也非字面 px） | 若 4-dev 把某段散文写到多段，正文可加 `max-width: 44em` 收行宽（中文 71 字/行偏长的逃生口，🟫 (b)） | 用 `em` 不触 U12 的 px 白名单，也不新增 token（`--cp-*` 语义不合用）。🟫 #4：本格原为 **2 格**、表头 3 列 → 渲染时「依据」列空、说明挤进「用途」，已补一个竖线对齐（GFM 表格里，行内 code 中的裸竖线字符仍算列分隔符，故此处用转义写法而非直接写那个字符） |
| `1000px` | 正文列 `max-width` | 1280 − 200(栏) − 2×40(--s10 留白) ≈ 1000 ⇒ **1280 视口零横向滚动**（NFR），且中文一行 ≈ 71 字 |
| `-9999px`（**必须负号**） | skip-link 移出屏（`:focus` 复位为 `left:var(--s2)`） | 可达性基线，非视觉值。🔴 三.6 实测：U12 的正则不吃负号 → `left:9999px`（正向）与 `left:-9999px` 在它眼里等价，而正向那一种会**撑出横向滚动**、直接撞 NFR「1280 无横向滚动」→ 新增 U23 机验禁正向 |

**已知例外（与 §3 hex 例外同族，写给 4-dev 与下一位评审）**：本稿**不承诺 WCAG 1.4.10 Reflow** —— 几何钉死 `200px + 1000px` 且不做断点，而「不承诺移动端」是**设备**豁免、不是 **200% 缩放**豁免（1280 放大 200% ≈ 640 CSS px 会出横向滚动）。理由：加断点要么引入第二套版心宽度（破 §6.2 白名单与「与产品侧栏同宽」的不可区分性），要么改用 `--cp-*` 那组语义不合的 token；两条都比这条 AA 缺口更贵。→ 归口 §14-⑩ 由需求侧裁决；本稿的裁量是**接受缺口并写明**，不是假装没有。

不做断点：NFR 明写「不承诺移动端」；`--cp-detail-width`、断点、`vw/vh` 一律不引入（第二栏在 1280 放不下 → **域目录进左栏，不做右侧 TOC**）。

**第二形态禁令（🔴 三.4/三.6 实测校准）**：clip 方案的 `width:1px;height:1px;clip-path:inset(50%)` 里的 **`width:1px` 会被 U12 捕获判红**（本轮实跑：探针 → 捕获 `1px`）→ 视觉隐藏只用上面那一行钉死的负号 skip-link 形态；确需第二种隐藏时单位改用 `em`/`%`（非 px → 不触 U12），别给白名单开后门。同时**校准 🔴 报的两条假红**：`border-width: 2px` 与 `@media (max-width: 500px)` 在当前 U12（词首边界 + 先排除 `:root`）下**已不被捕获**（实测：探针捕获值为空），那是 §13.1-2 修口之前旧式的症状。

**触摸目标实测**：左栏项 `13px/1.5` 行高 19.5px + 上下 `7px` → **19.5 + 14 = 33.5px** 高（🔴 二轮纠正：我原先写 35.5px 是按 8px 算的，算术错；`7px` 是 `App.tsx:287` 的逐字值，走 §6.1 例外集，两处结论都不变）→ WCAG 2.5.8（24px, AA）✅；2.5.5（44px, AAA）不达 → 登记 §14-⑩，不为 AAA 破坏与产品侧栏同宽的几何。徽标 chip 非控件不受该条约束。
...
- 滚动定位反馈由**静态描边**承担，且必须用**看得见的那一档**：`.domain:target, .page-block:target, .notice:target { outline: 2px solid var(--blue); outline-offset: 2px; background: var(--bg-selected) }`。理由：`:target` 是本稿**唯一**的跳转反馈通道（无按钮、无动画、不做当前项高亮），而 `--blue-border` 叠底实测 **1.30:1（on `--bg-app`）/ 1.33:1（on `--bg-card`）**，与 `--border`（**1.18**，§4 已按 🔴 复算改正）/`--border-strong`（1.34）同档 → 用它等于违反 §3.1 hairline 行我自己写下的「不足 3:1 不得用于表达交互态」（🟫 ① 与 🟦 阻断一独立复算同判）。改用焦点已用的 `--blue`（5.29/5.87:1 ≥ 3:1，与 §7 N7 的 `:focus-visible` 同形、`outline` 不动布局）；`--bg-selected` 底**保留**为第二通道（叠底 ΔL +0.057/+0.049，暗色分层阈值 0.04 以上），但**判定不依赖它**；新增 U14 机验 `:target` 里禁现 `--blue-border`。
...
|---|---|---|---|
| **C0 文档根** | `<html lang="zh-CN">` + `<head>` 内 `<meta charset="utf-8">` + `<title>AgentFlow · 产品原型与界面原型文档</title>` | 三件逐字取产品形态（`frontend/index.html:2/4/6` 实测就是这三件），品牌名按 §1 假设 6 走 `README.md:1` 口径，不新造 | **WCAG 3.1.1 / 2.4.2 两个 A 级**：中文正文 + 等宽英文路径混排，无 `lang` 时 NVDA/JAWS 按英文声库读中文＝不可懂，§9 整列「读屏」承诺同时落空；无 `<title>` 时读屏入场与标签页只剩文件名（本稿定性"天生会被转发"）。U18 期望 1、U19 期望 ≥1 —— 两条都是**基线缺口**而非样式偏好：U1…U17 与 13 条 AC 无一能抓到它 | 
...
| **C12 SVG 框图** | `<svg viewBox class="fig">` + `<g class="node">` / `<path class="edge">` | `.node{fill:var(--bg-card);stroke:var(--text-secondary)}`（🔴 缺口 5：原钉的 `--border-strong` 在 `--bg-input` 画布底上实测 **1.27:1**，节点边界属 1.4.11 要求的图形对象 → 换 6.85:1；`.edge{stroke:var(--blue)}` 5.94 保持不变） `.edge{fill:none;stroke:var(--blue);stroke-width:1.5}` —— **SVG 内不写字面色、不写 `var()` 进 presentation 属性**（浏览器不可靠），一律走类 | 无 `<use href>`（AC-3 正命门）、无 `data:` 位图、`<title>`/`role="img"`+`aria-label` 给读屏器；两张为限（D3 框图 + §3 状态机） |

| **C13 左栏项** | `<a class="toc-entry" data-nav="<label>" href="#pg-<Component>">`（第一组）／`<a class="toc-entry toc-domain" href="#sec-*">`（第二组）／附录两条同形；**可见的 `💬` 包一层 `<span aria-hidden="true">`**（🔴 建议采纳：免英文声库念出 "speech balloon"；`data-nav` 属性原文不动 → K5/AC-2 仍绿。据实：**产品侧 `aria-hidden` 零命中**（实测 `grep -rn 'aria-hidden' frontend/src` → 0），这是本稿自补，不是沿用） | `display:block; padding:7px 10px; margin-bottom:2px; border-radius:var(--r-sm); font-size:13px; color:var(--text-secondary)` —— 🟫 #3：`7px 10px`／`margin-bottom:2px` 确实**跳出 §6.1 的 8 档闭集**，我选她给的 **(b)**：保留**逐字同形**（出处 `App.tsx:287/316/341` 的 `padding: collapsed ? '8px 0' : '7px 10px'`、`:288/317` 的 `marginBottom: 2`）＋ §6.1 明列例外集 ＋ **U25 机验**；不选 (a)（改回 `--s2/--s3`）是因为那会把左栏从"跟后台一模一样"换成"照 scale 但认不出"，而 §6.1 的白名单守不住的那一半交给闸、不交给散文；栏题 11px/600；hover 只动 `background:var(--bg-card-hover)`+`color:var(--text)`（与产品 `.card-clickable:hover` 同族信号，🟫 (c) 见下） | **`href` 是接口的一部分**：无 `href` 的 `<a>` 不是链接、不进 tab 顺序（N7/U15 机验）；`data-nav` 仍只许第一组用（K5/N1）；U16 机验每条 `href="#x"` 都能在同稿找到 `id="x"` |
...
|---|---|---|---|---|---|---|
| 左栏入口 / TOC 项 | 13px `--text-secondary`，无底 | 底 `--bg-card-hover` + 字 `--text`（100ms 颜色） | 2px `--blue` 环 + 2px offset | 静态稿无路由态 → **不做当前项态**（N5） | Tab 逐项，Enter 跳锚 | `nav[aria-label="文档目录"]` 内链接；`💬` 已包 `<span aria-hidden="true">` → 读屏读「对话中心」（U24；可见文本与 `data-nav` 原文不动）。🔴 二轮指出：这一列在我改标记后仍写「原文照读」＝承诺与形态不一致，已对齐 |
...
| **E** | **我自己触发的 AC-12 红（已修，留在这是为了让"跑解析器"这件事留下代价记录）**：回写票面时我用 `/opt/python3.14/bin/python3` 直接 import `backend/parsers/gate.py` 复验票面块 → 生成 `backend/parsers/__pycache__/gate.cpython-314.pyc`，随后的 `git add -A` 把它一起提交 → AC-12（零 L1 写入）在 `e419b88f` 之后实测 **1** | `a6a73506` 删除该产物，`b15c4554..HEAD` 净差异回到 **0**（AC-12 复跑 ✅）。纪律：**读产品代码的解析器要在仓库外副本里跑**，或跑完立刻清 `__pycache__` 再 `add`；`.gitignore` 补 `__pycache__/` 是 L1 写入，我不做 → 归口 §14-⑧ | 无（本阶段自纠）。🟫 #4：本行原为 **3 格**、表头 4 列，已补 |
| **F** | **A 级基线漏在观察之外**：§0.2 八维最初没读 `frontend/index.html`，于是 `<html lang="zh-CN">` / `<meta charset>` / `<title>` 这三件**产品侧已有**的东西没进契约，五份工件里 `grep -rn "lang=\|zh-CN\|xml:lang"` → 0 命中（🔴 缺口①②） | 新增 C0 文档根 + U18（期望 1）/U19（≥1）；§0.2 补「文档根」观察行 | 无（本阶段自纠） |
| **G** | **§4 基准表我自己算错一个数**：`--border` on `--bg-card` 我写 **1.06**，🔴 复算 **1.18**（on `--bg-app` 1.14）——我用同一 WCAG 相对亮度法复算，确认她对我错（alpha 合成底算错） | 表值改正 + 追加 4 行非文本对比（1.4.11 视角）；结论方向不变（都远低于 3:1） | 无（本阶段自纠） |
...
## 13. UI 侧自检闸 U1…U27（含 U2a 空洞自检）（设计侧 K 系列的视觉补充；**本轮逐条实跑**，命令单行、可原样抽出跑；🟫/🟦 的票内修法已并入）
...
# (14) :target 禁再走任何不可见档描边（🟫 ① ＋ 🟦 阻断一；🔴 二轮扩式：原式只认 --blue-border，:target{border-color:var(--border-strong)}（实测 1.34:1）会放过）
grep -oE ':target[^}]*\}' "$F" | grep -cE 'var\(--(border|border-strong|blue-border)\)'
...
# (17) 打印不承诺必须显式声明（🟦 非阻断3）
grep -c '不支持打印' "$F"
# (18)(19) A 级两件套：lang 与 文档 title（🔴 缺口 1/2；期望 1 与 **head 窗口内 1**）
grep -c '<html[^>]*lang=' "$F"
sed -n '/<head>/,/<\/head>/p' "$F" | grep -c '<title>[^<]'
# (19b) 诊断行（不是判据）：全文 <title> 计数＝文档 + C12 的 SVG 各一 → 2。🔴 二轮实测：U19 原式（全文 ≥1）在"只删文档级 <title>、留 SVG <title>"时仍给 1＝A 级 2.4.2 假绿 → 判据改走 head 窗口（正例 1／该负例 0），原式降为诊断
grep -c '<title>[^<]' "$F"
# (20) 每枚徽标都有可读文案（🔴 缺口 4 的等价式：短词已改文本节点，空 chip 即缺陷；期望 0）
grep -oE '<span class="chip[^>]*>[^<]*</span>' "$F" | grep -cE '>[[:space:]]*</span>'
# (21) 状态与图形的描边禁低对比档（🔴 缺口 5；期望 0。🔴 二轮改窄式：原式全文扫描 → C4 o4/s1-s3 与 C6 的**合规静框**（§8 钉的 `--border-strong`）也判红，同一视觉换 `border:` 简写又逃过＝判据不可复现。改为只看状态/图形选择器）
grep -oE '[^{}]*\{[^}]*\}' "$F" | grep -E '(:target|:hover|:focus|\.node|\.edge)' | grep -cE '(border-color|stroke): *var\(--(border|border-strong|blue-border)\)'
# (21b) 载体等值：data-nav 只能长在 <a> 上（🔴 二轮；期望 PASS —— 换成 <span data-nav> 时 U15/U15b 会双双空过，而"13 项不进 tab 序"这个缺陷正好回来）
test $(grep -c 'data-nav=' "$F") -eq $(grep -oE '<a\b[^>]*>' "$F" | grep -c 'data-nav=') && echo PASS || echo RED
# (20b) 徽标等值：chip 必须是"开标签 + 纯文本 + 闭标签"（🔴 二轮；期望 PASS —— 内嵌 <b> 会让 U20 的 -oE 捕获失败＝静默逃逸，既不红也不绿）
test $(grep -oE '<span class="chip' "$F" | wc -l) -eq $(grep -oE '<span class="chip[^>]*>[^<]*</span>' "$F" | wc -l) && echo PASS || echo RED
# (22) 每张表都有 caption（§9 承诺过、原先无断言；期望 PASS）
test $(grep -c '<table' "$F") -eq $(grep -c '<caption' "$F") && echo PASS || echo RED
# (23)(23b) skip-link 只能负向移出屏（正向会撑出横向滚动，撞 NFR；期望 0 与 >=1）
grep -cE 'left: *9999px' "$F"
grep -c 'left:-9999px' "$F"
# (24) 可见 emoji 对读屏隐藏（🔴 建议；期望 >=1）
grep -c 'aria-hidden="true">💬' "$F"
# (15b) 🔴 核式的**顺序无关**替代（期望 0；她的原式在本稿形态下是空洞检查，见 §13.2）
grep -oE '<a\b[^>]*>' "$F" | grep 'data-nav=' | grep -vc 'href="#'
```
...
| 闸 | 结果 | 闸 | 结果 | 闸 | 结果 |
|---|---|---|---|---|---|
| U3 幻影变量 | ✅ 0 | U4 章外域名 | ✅ 0 | U5a / U5b 嵌套截断 | ✅ `PASS 120<122` / `PASS 97<104` |
...
| U12 几何白名单 | ✅ 空（补集 172 行，非空洞） | U13 体积 | ✅ 30 793 B（预算 200 KB 的 15.3%） | U9b 滚动动画 | ✅ 0 |
| U14 `:target` 禁三档低对比描边（🔴 扩式） | ✅ 0／负例 `border-color:var(--border-strong)` → 1 | U15 `data-nav` 必带 `href` | ✅ 0 | U16 锚点落点 | ✅ 空 |
| U17 打印声明 | ✅ 1 | U18 `<html lang>` | ✅ 1 | U19 文档 `<title>`（🔴 head 窗口式） | ✅ 1／删文档 title 负例 → 0 |
| U20 空 chip | ✅ 0 | U21 低对比描边（🔴 窄式：只看 `:target`/`:hover`/`:focus`/`.node`/`.edge`） | ✅ 0／`.node` 回退负例 → 1 | U22 表格 `caption` | ✅ PASS（2==2） |
| **U21b 载体等值** | ✅ PASS／`<span data-nav>` 负例 → RED | **U20b 徽标等值** | ✅ PASS／`<b>已有</b>` 负例 → RED |
| U23 skip 正向 | ✅ 0 | U23b skip 负向 | ✅ 1 | U24 emoji `aria-hidden` | ✅ 1 |
| **U2a 排除式空洞自检** | ✅ `PASS 172/257` | **U25 间距闭集** | ✅ 无输出 | **U26 短词限两字** | ✅ 无输出 |
| **U27 `--blue-border` 零引用** | ✅ 0 | | | | |
| U15b 顺序无关核式 | ✅ 0（负例注入 → 1） | | | | |
...
### 13.2 🔴 无障碍专家第 3 条核对命令的精度问题（她的修法我照抄，只有这条命令按她的意图改稳）

她的核对式是 `grep -oE '<a [^>]*data-nav="[^"]*"' "$F" | grep -vc 'href='`。问题出在 `-o` 的截断点：本稿（和她建议的出路 (a)）把 `href` 写在 `data-nav` **之后**，所以捕获串永远停在 `data-nav="…"` 的收尾引号处，`href=` 天然不在捕获里 —— **本轮实测**：合规样本 → **13**（应为 0，全红）；我再把其中一条 `href` 删掉造负例 → 她的式子给 **0**（应为 1，全绿）。两个方向都反，等于一个恒真的假闸。
她的本意（无 `href` 就不算链接）完全成立，我照原意实现成顺序无关式并双向验证：

```bash
grep -oE '<a\b[^>]*>' "$F" | grep 'data-nav=' | grep -vc 'href="#'   # U15b：正例 0 / 删掉一条 href 的负例 1 ✅
```
（U15 也保留，它取整个 `<a …>` 标签，同样顺序无关。）这不是对她的否决 —— 她的缺口③我已按出路 (a) 落地（`href` 真指到 `#pg-*`，N8 现算 + C13 钉形态），只有这条命令的捕获形态需要跟着属性顺序走；请她在新 tip 上用 U15/U15b 复跑。

### 13.3 排除式 `:root` 窗口曾**静默空洞**（本轮我自己抓到，🟩 #1 那一族的第二例）

`U2b`（根外色值）与 `U12`（版心字面量）都写成 `sed -n '/^:root *[{]/,/^ *}/!p'`。§13.1-2 把锚改宽容之后，它在我那份样本上**仍然不成立**：样本当初把 `:root` 写成紧凑多行、**收尾 `}` 不独占一行** → 结束地址永不匹配 → sed 把"从 `:root` 到文件结尾"整段当成范围内 → 补集只剩 **2 行 / 179 行**，于是"根外零色值""版心零脏值"两条**都是空洞绿**（它们只看见 `:root` 之前那 2 行）。本轮改成**逐字节照抄 `tokens.css:6-90`**（收尾 `}` 独占一行，也正是 `T-UI-01` 要求 4-dev 做的事）后，补集 **172 行 / 257 行**，两条闸第一次真正扫到全部 CSS：`U2b` 仍 **0**、`U12` 仍 **空**、`U25` 扫到的字面量恰为例外集六个值 ✅。

新增 **U2a 空洞自检**（把"排除式闸到底排掉了多少"变成一条会红的断言，防止下一次再静默）：

```bash
B=$(awk '/^:root *[{]/{r=1} r{n++} r&&/^ *}$/{print n; exit}' "$F"); C=$(sed -n '/^:root *[{]/,/^ *}/!p' "$F" | wc -l); T=$(wc -l < "$F")
test $((T-B)) -eq $C && echo "PASS $C/$T" || echo "RED $C/$T B=$B"     # 实测 PASS 172/257
```
**给 4-dev 的硬约束（进 `T-UI-01`）**：`:root` 必须**整块逐字节照抄**，含**收尾 `}` 独占一行**；把它压成紧凑写法不会让 K6b 变红，却会让 §13 里三条排除式闸一起失效。

### 13.4 🔴 无障碍专家二轮回执（改票 ✅）与 5 条加严式的落地数字

她按**自己重建的探针**复跑（我的样本在仓库外，她复跑不了，这个口径她先说清了），第 3 条裁定「**你对，我的式子作废**」，并给出三处「闸还能被绕过」＋两处「一假绿一假红」。我在**本稿样本**上把她五条式子**双向**各跑一遍（正例＝现钉形态，负例＝一次只破坏一件），全部按她给的形态落地：

| 她的加严式 | 我的正例 | 我的负例（注入缺陷） | 落地 |
|---|---|---|---|
| `U19` 改 head 窗口（原 ≥1 式被"只剩 SVG `<title>`"糊过去：正例 2 → 删文档 title 仍 1＝假绿） | **1** | 删文档级 `<title>` → **0** ✅ 会响 | 命令块 (19) 已换式 |
| `U14` 扩到三档描边（原式只认 `--blue-border`，`:target{border-color:var(--border-strong)}` 1.34:1 放过） | **0** | 注入该缺陷 → **1** ✅ | 命令块 (14) 已换式 |
| `U21` 改选择器窗口（原式全文扫 → §8 钉的合规静框判红 3 处，而 `border:` 简写又逃过＝同一视觉两种判法，不可复现） | **0** | `.node` 描边回退 `--border-strong` → **1** ✅ | 命令块 (21) 已换式 |
| **U21b** 载体等值（13 项若从 `<a>` 换成 `<span data-nav>`，U10 仍数 13、U15/U15b 因 `<a\b` 抓不到标签而双双空过＝我原报缺陷正好复活） | **PASS** | span 形态 → **RED** ✅ | 新增 |
| **U20b** 徽标等值（`<span class="chip…"><b>已有</b></span>` 让 U20 的 `-oE` 捕获失败＝静默逃逸，不红也不绿） | **PASS** | 嵌套 `<b>` → **RED** ✅ | 新增 |

两点如实分岔：① **U21 窄式在我样本上 0**，她探针里剩下的 1 是 `.card:hover{border-color:var(--border-strong)}`（她按 §0.2 产品 hover 语汇建的探针）——本稿该形态**不存在**（🟫 (c) 已取消页块/待标注 hover），所以我不必做"留哨还是放行"的裁量；② 她四-2 的"§13 一条间距字面量闸都没有"在**她复验的 tip `516b0e7a` 上成立**，但 `834b5399` 已按 🟫 #3 落了 **U25**：她的式子实测只取声明**首值**（本稿报 `1 2 3 7`，`7px 10px` 的 `10px` 与 `.badge` 的 `6px` 漏），我的 U25 用 `[^;}]*` 吃整条声明（本稿无输出、负例 `margin-bottom:5px` → `5px`），且带 §6.1 六个有出处的例外 → **保留 U25**，两条不冲突。她给的 `--blue` 压 `--bg-selected`∘`--bg-card` = **4.65:1** 我用同法复算一致（第一次代 0.08 得 4.78，按 `tokens.css:15` 真值 0.1 才是 4.65）→ 已入 §4 表。她自己被证伪的两条（`border-width`／`@media max-width` 假红已随 §13.1-2 消失、`--text` 压白底 1.30 她认我对）她已在一轮背书，本轮再确认，不再重述。
...
**同一份样本上的已投票票面 + 设计侧 K 系列（本轮实跑逐条，零红）**：`AC-1 32/168 · AC-2 32 且入口 diff 空 · AC-3 0 · AC-4 三层全 0 · AC-5a 0 / AC-5b 33/33 · AC-6 行在（`834b5399` · 2026-09-22 · 页面 32 / 端点 168 / 域 11，样本重建时点现取） · AC-7a 1 / AC-7b 0 · AC-8a 1 / AC-8b 1 · AC-9 1 · AC-10 16 0 · AC-11 7(≥6) · AC-12 0（本轮曾红一次，见 §10-E）· AC-13 9 · K1 稳健式双向空 · K2 空 · K3 空 · K3b 空 · K4 空（可见子集 12） · K4b PASS · K6 6 · K6b 空 · K6c 12 · K7 1 · K8 0 · K9 空 · K10 1 · K11 0 · K12 1 · U1…U17 全中（U5a 120<122、U5b 97<104、U9b 0、U10 13、U13 30 793 B、U14 0、U15 0、U15b 0、U16 空、U17 1、U18 1、U19 1（head 窗口）、U20 0、U20b PASS、U21 0、U21b PASS、U22 PASS、U23 0、U23b 1、U24 1）`；另附四行**诊断/证据**（K1c 现式左集 33 项 → §10-D；U2c 真身 `:root` 块严格锚 31、U12b 旧式脏值 420px → §13.1；**U19b 全文 `<title>` 2 处** → §13.4）。整轮 **78 行跑批 = 74 行断言 + 4 行诊断**（🔴 二轮再钉两条等值式、改稳三条既有式；口径修正：原先写「75 行断言」把 3 行诊断混进了断言数，这次分开计）（🟦 要求把 U14 与两条锚点检查计入 → 已计；🔴 要求把 `lang`/`<title>`/可聚焦/徽标覆盖/低对比描边计入 → 已计为 U18–U24），含 1 行 K4b 与 2 行 U5 的 `PASS` 文案式输出。
...
| ⑩ | **两处 AA/AAA 缺口是有理由的接受，不是遗漏**：(i) 1.4.10 Reflow —— 200% 缩放（1280→640 CSS px）下固定 `200px+1000px` 版心会出横向滚动，NFR 的「不承诺移动端」是设备豁免不是缩放豁免；(ii) 2.5.5 触摸目标 AAA —— 左栏项实测 **33.5px**（19.5 + 7×2），2.5.8(AA/24px) 已过、AAA(44px) 不达 | 修法都要付更大代价：断点需第二套版心宽度（破 §6.2 白名单与「与产品侧栏同宽」），44px 目标高会破坏左栏密度。已在 §6.2 写成显式已知例外 | 需求分析（要不要为缩放承诺一套响应式版心）+ 资深用户体验官（44px 一档是否纳入本产品的 AA+ 基线） |
| ⑪ | **D13 的适用范围需要上游写清**：`DESIGN:105` 的 D13 仍写「可见徽标文案由 CSS 的 `::after` 配语义 class」，而 🟫 ④ 已把可见文案改成文本节点（Ctrl+F 可搜）。D13 真正锁的是「**每行只出现一次**」（堆叠即红，K4b/AC-5b 机验），**可见文案的形态属 2a 主责** | 我不改上游工件（R3：不改已投票判据）；本轮只把 §8/§9/`T-UI-05` 三处对齐并加 U26 机验 | 任务拆解 → 由 0-change/架构设计在 D13 补一句「锁单写，不锁 CSS 生成内容形态」（🟦 #1 提出） |
...
|---|---|---|
| `T-UI-01` | **`:root` 整块逐字节照抄 `tokens.css:6-90`（含收尾 `}` 独占一行）**——压成紧凑写法不触 K6b 但会让 §13 三条排除式闸一起空洞（§13.3），U2a 会红 | C0 文档根三件先落地（`<html lang="zh-CN">` + `<meta charset="utf-8">` + `<title>`，逐字取 `index.html:2/4/6`）＋ 把 §3.1 的 token 子集**逐字节**物化进交付稿 `:root`（含 `'Segoe UI'` 单引号形态），只声明用到的名 | `comm -13 <(tokens.css 变量对) <(稿内变量对)` → 空（K6b） |
...
| `T-UI-04` | C1/C2/C3 + C13 左栏项（13 条 `href="#pg-*"` 按 N8 现算；可见 `💬` 包 `aria-hidden`）+ `:target` 用 `--blue` outline + 域章属性顺序（`class` 前 `id` 后）+ 域章内顺序钉法 + 页集覆盖 | `grep -c 'class="domain"' → 12`（K6c）+ U14 0 + U15 0 + U15b 0 + U16 空 + U18 1 + U19（head 窗口）1 + U21b PASS + U24 ≥1 + **U25/U27 无输出与 0 + U2a PASS** + U5a/U5b PASS + **K1 用稳健式**（先 `sed -E 's/（[^（）]*）//g'` 剥全角括号注解、再 `tr '、' '\n'`）→ `comm` 双向空；用现式会得 33 项假红，见 §10-D |
| `T-UI-05` | C4/C5 两轴徽标：**短词文本节点** + o1-o3 走产品 `.badge` 同色 8% 底原形（禁 `::after`/`role="img"`/`aria-label`） | U6a 0 + U6b 0 + **U20 空 chip 0 + U26 限两字无输出**（🔴 缺口 4 的等价覆盖断言）+ AC-5b occ==lines（33/33）+ 双写原串必红（反例实测 `occ 5/lines 3`） |
| `T-UI-06` | C6/C7 待标注与边界句（6 个具名 `nb-*` 落所属域章、且早于任何嵌套 section） | K6 → 6 + AC-7a ≥1 + AC-8a/b ≥1 |
| `T-UI-07` | C8/C9/C10（含短词↔原串映射）/C12（9 个 `vb-G*`、12 slug 行、图例、两张 SVG 框图；`.node` 描边走 `--text-secondary`） | K9 diff 空 + K4 diff 空 + **U21 0 + U22 PASS** + AC-13 → 9 + K11 → 0 |
| `T-UI-08` | 可达性收尾 + C14 打印声明行：skip-link（**负号 `left:-9999px`**，U23/U23b）/ `:focus-visible` / `nav[aria-label]` / `caption+th[scope]` / reduced-motion 兜底 / `<details>` / C0 文档根 | **U9 ≥1 ＋ U18 1 ＋ U19（head 窗口）1 ＋ U22 PASS ＋ U23 0 ＋ U23b ≥1 ＋ U24 ≥1**（🔴 二轮：可达性收尾任务的验收列原来一条可达性断言都没挂）＋ 原 U7 0 / U8 空 |
| `T-UI-09` | 全闸复跑并回填票面（DESIGN §1.5 现算的已投票 AC 与 K 系列闸 ＋ 本文件 U1…U27），样本一律落仓库外 | 全部断言中且零红 + `git status --porcelain` 不含样本 |
...
- [x] **🔴 五条缺口已落地**：C0 文档根（`lang`/`charset`/`<title>`，逐字取 `frontend/index.html:2/4/6`）＋ U18/U19；键盘路径按她的出路 (a) 落地（N8/C13/U15/U15b/U16，负例注入证明闸会响）；徽标覆盖改 U20 等价式（她的 `aria-label` 覆盖率式在文本节点形态下会把合规稿判红，理由与复现见 §13.2 与本轮实测）；`:target`→`--blue`（U14）与 `.node`→`--text-secondary`（U21）；caption 覆盖补 U22；另采纳 emoji `aria-hidden`（U24）与 skip-link 负号（U23/U23b）。非阻断六条逐条写进 §4/§6.2/§10-G/§14-⑩，其中两条我复算后**证伪了它的当前影响**（`border-width`/`@media max-width` 假红已随旧式修口消失，实测探针为空），如实写明而不是照抄。
- [x] **票内修口已落地**：🟩 一票附的 5 条 U 闸精度/口径修口逐条复现后落地（§13.1 有修前→修后的实跑数字），未触碰任何已投票 AC 或冻结的 K 命令。
- [x] **门禁**：G2a **4/4 通过** —— 🟫 ✅（维持）· 🟦 ✅（复跑改票）· 🟩 ✅（5 条修口落地）· 🔴 ✅（五条缺口逐条复跑会响后改票，另给 8 条残留已全收，§13.4）→ **本阶段出口成立**；3-task 交接已在上一笔评论 @任务拆解 完成，本轮不重复唤醒；四位的复算数字与本轮 **78 行跑批（74 断言 + 4 诊断）**一并留档
...
🟦 资深用户体验官: ✅ 改票（首轮 ❌ 的两条阻断由她自己在探针上复跑 U14 0／U15 0／U16 空、N8 十三落点独立复算全命中、§13.0 六条现算 32/13/32+168/9/12 全中后撤回反对；首轮 ❌ 的理由与修法留在评论，不抹平条件）；她另给两处一行级残留（C4 末列 `::after`、U6a/U6b 成空洞护栏）→ 本轮与 🟫 #1/#5 一并落地并补 U26
🟩 前端架构师: ✅ token 封闭性/对比度/锚点逐值复算一致，§10-D 独立复跑同判（现式 33／稳健式 32），C1–C12 零 JS 零构建可表达；附 5 条一行级 U 闸精度修口（已全部落地并复跑，见 §13.1）
🔴 无障碍专家: ✅ 改票（二轮在她自己重建的探针上逐条复跑我落地的五条缺口，判定「无障碍合规等级：✅ 达标（AA ＋ §14-⑩ 两条有理由的已知例外）」；她第 3 条核式作废、以我的 U15b 为准；另给 8 条不门控本票的残留，本轮全部落地，见 §13.4）
结果: **4/4 通过** → 按 R13 表 ✅ 自动进入下一阶段，不问「要继续吗」。三处时点差已对齐：🟦 于 18:49 改票 ✅（我 18:59 那笔回写写的是她改票前的 §16 快照），🔴 于本轮改票 ✅ → 本门 **4/4**。反对意见存档仍留在 §18（按 R13 保留记录，不改写历史）

---

## 17. 本门票面为什么落在这个工件里（反对意见存档见 §18，在本节之后）
...
## 18. 反对意见存档（写在 3/4 时点、按 R13 要求留档；🔴 已于本轮在她的探针上逐条复跑后改票 ✅ → **本门最终 4/4**，本节不改写历史，只把状态更新为「异议闭环」）

**🔴 无障碍专家 ❌ → ✅（本门收口：4/4）**（她复验到 `486453d5`，早于本轮修法）五条缺口与当前状态：① `<html lang>` 缺失（3.1.1 A 级）→ **已落地** C0 ＋ U18（实测 1）；② 文档级 `<title>` 缺失（2.4.2 A 级）→ **已落地** C0 ＋ U19（实测 2）；③ 键盘路径在 `<a data-nav>` 无 `href` 的写法下不成立（2.1.1/2.4.3/2.4.4）→ **按她的出路 (a) 落地** N8/C13/U15/U16（该点与 🟦 阻断二同源，R9.2 二次确认成立；她给的核对命令属性序敏感，改稳与复现见 §13.2）；④ `::after` 单写下漏一个 `aria-label` 会静默丢失（1.1.1/4.1.2）→ **形态已被 🟫 ④ 换成文本节点**，她的覆盖率式会把合规稿全判红，等价式 **U20**（空 chip 0）＋ **U26**（短词限两字）替代，**她二轮改判：文本节点形态比她原式更强**，并要求补 **U20b** 等值式关掉嵌套逃逸（已落，§13.4）；⑤ 状态与图形的非文本对比 1.27–1.33 破本文件自订的 3:1（1.4.11）→ **已落地** `:target`→`--blue`（U14）＋ `.node`→`--text-secondary`（U21）＋ §3.1 角色行 ＋ **U27** 全稿零引用。她二轮回执又给 8 条（5 条加严式 + 2 格措辞/算术 + 1 条任务挂载），本轮全收并双向机验，见 §13.4；下面的六条登记仍按原样存档。她另登记的六条非阻断：`scroll-behavior` 出处与 smooth（上一轮随 🟫 ② 已改）、`--border` 1.06→**1.18**（她复算纠正我 → §10-G）、1.4.10 Reflow 与 2.5.5 AAA 写成有理由的已知例外并归口 §14-⑩、两条形态坑（焦点环宽度只能写 `outline:`、skip-link 必须负号）已钉死并加 U23/U23b、emoji `aria-hidden` 采纳（U24）。

**其余三位的背书与保留**：🟫 对 §4 全部 17 行对比度逐行复算一致、维持 ✅ 并另给 6 处一格级残留（本轮全收）；🟦 交互可用性判 **A-**（扣的半档是"反馈全押在静态描边上"，她认这个赌注），并撤回她首轮的"聚合视图"建议 → **3-task 不必为此新增任务名额**；🟩 对 token 封闭性、对比度与锚点逐值复算一致，附 5 条一行级 U 闸精度修口（全部落地，§13.1）。
