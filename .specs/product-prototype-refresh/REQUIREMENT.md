...
### AC-2 · 32 页全覆盖、逐页可指域
- **Given** 交付的原型稿 + BASELINE §2 的 32 行页面归属表
- **When** 人工按该表逐行在原型中找到对应页面块
- **Then** 32 行全部命中、无缺页；每行显示所属域
- **验证方式**: UAT-1（一次人工走查，拿 §2 表逐行勾）＋ `grep -c 'class="page-block"' product-design.html` → `≥32`
...
### AC-11 · 未接入能力不得画成已交付
- **Given** 原型涉及 BASELINE §3/§4 点名的四处「壳能力」：token 统计、MCP 工具、调用链查看（`TraceViewer` 零挂载）、知识库检索（无 embedding）
- **When** 呈现它们
- **Then** 每处带「未接入 / 演示边界 / 规划中」三态之一标注，孤儿页面（2 个）不得作为能力页出现
- **验证方式**: `grep -cE "未接入|演示边界|规划中" product-design.html` → `≥4`
...
> 预留：`🗳️ 需求门: ...` 四票与结果；3/4 通过时反对意见抄录于此。
