...
| D19 | **真源全集 ≠ 交付稿可见集时，闸按「可见子集」派生**，差值显式登记在闸行里（`grep -v <不外显 slug>`），**禁止**用「把内部件条目画进交付稿」来凑绿 | ① 左集照全集算，逼 4-dev 去补第 13 条 slug；② 或现场把 K4 改成「稿子有什么就算什么」（右集也松掉） | 🧭 交付总调度点名要求闭掉（截止＝4-dev 写稿前）：旧式对合规稿**必红**，而消红最省事的路径恰好是把 S11 那条任意文件读的细节写进外发交付稿——正是 ADR-001／D11 明令禁止的动作，**闸口自己把执行者往禁止方向推**（反向激励留到实现阶段现场发现太贵）。本轮双向实测：合规稿旧式红 `缺 admin-file-read-jail`、红稿 13 条旧式反而空；新式两侧判对（12 绿 / 13 红 `> admin-file-read-jail`） | 多一处必须手工维护的排除项 → 用 §7.6 (9) 组的双向断言看着它；真源若再加不外显条目，须同笔改 K4 与 RESEARCH §2 两处 |
