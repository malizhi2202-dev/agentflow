...
| 竞品结论与去向 | 有：`RESEARCH-competitors.md` §2（G 号 + 去向 + slug 唯一真源） | **派生**（K4 现算 9 条融入集与 **12 条可见 slug 集**；真源 13 条，差值见 D19），禁重算、禁重述方案 |
...
| D14 | 交付稿**必须完整呈现 12 个可见 slug**（真源第 13 条 `admin-file-read-jail` 属内部件，依 D19 **不进稿**）（`data-slug` 或 `<code>` 原样，AC-10 的 v2 呈现要求），K4 才能拿 `RESEARCH §2` 现算集的**可见子集**做双向 diff | 只写"另有 12 条议题见 RESEARCH" | 只写指句 → K4 无从 diff，"写了没落"重新变成无人验的口子（正是 AC-13 ③ 记录过的那类失效） | 交付稿多一张 12 行表；人工若砍扩展候选（人工 ②），该表要跟着回算 |
...
| D18 | **闸清单禁写范围号，一律引用式**：非 §1.5 处一律写「K 系列闸（§1.5 现算）」，不写 `K1…K<n>` | 每处手抄当前范围 | G2 收口轮 🟩 抓到：我为加 K12 而 sed 过一轮范围号，结果 `DESIGN` 七处 ＋ `ADR-002` 一处仍写 `§1.5 现算的 K 系列` ——**这正是 D2「反对手抄」的微观实例**，只不过抄的不是数字是编号范围；范围号一过期就误导 2a/4-dev 少跑几条闸 | 引用处不如具体范围号直观，须靠 §1.5 的标题定位 || D19 | **真源全集 ≠ 交付稿可见集时，闸按「可见子集」派生**，差值显式登记在闸行里（`grep -v <不外显 slug>`），**禁止**用「把内部件条目画进交付稿」来凑绿 | ① 左集照全集算，逼 4-dev 去补第 13 条 slug；② 或现场把 K4 改成「稿子有什么就算什么」（右集也松掉） | 🧭 交付总调度点名要求闭掉（截止＝4-dev 写稿前）：旧式对合规稿**必红**，而消红最省事的路径恰好是把 S11 那条任意文件读的细节写进外发交付稿——正是 ADR-001／D11 明令禁止的动作，**闸口自己把执行者往禁止方向推**（反向激励留到实现阶段现场发现太贵）。本轮双向实测：合规稿旧式红 `缺 admin-file-read-jail`、红稿 13 条旧式反而空；新式两侧判对（12 绿 / 13 红 `> admin-file-read-jail`） | 多一处必须手工维护的排除项 → 用 §7.6 (9) 组的双向断言看着它；真源若再加不外显条目，须同笔改 K4 与 RESEARCH §2 两处 |
...
K4  diff <(grep -oE '`[a-z]+(-[a-z]+)+`' .specs/product-prototype-refresh/RESEARCH-competitors.md | sort -u | grep -v human-approval \
               | grep -v admin-file-read-jail) \
         <(grep -oE 'data-slug="[a-z-]+"' "$F" | sed -E 's/data-slug="([a-z-]+)"/`\1`/' | sort -u)      # 空 = 可见子集 12 条齐
    # D19：真源全集与交付稿可见集**可以不等**。`admin-file-read-jail`（= BASELINE 的 S11）是内部件条目，
    # 依 ADR-001／D11 不得进这份天生外发的稿子 → 左集按"可见子集"现算（真源现算 13，减不外显 1 = 12，
    # 与 `RESEARCH §2` 计数不变式块同一口径、同一表达式）。差值必须显式写出，禁靠"稿子刚好等于全集"蒙混。
    # 🧭 交棒时点：截止是"4-dev 写交付稿之前"，不等 2a。旧式两头都错（本轮实测）：对 12 条的合规稿报
    # `缺 admin-file-read-jail`（假红），对把 S11 画进稿子的 13 条红稿反而报空（放红）——修口方向唯一：收窄左集，不放宽比对。
...
- **`admin-file-read-jail`（🔴 B-2 登记的需代码议题，现已并档为真源第 13 条）**：`admin_api.py:126-135` 的 `GET /api/admin/files/{path:path}` 无鉴权 ＋ 绝对路径绕过 `..` 守卫 → 任意文件读；修法是 GET 补 `get_current_user` ＋ `full.startswith(_prompts_dir())`（同文件 PUT `:139-143` 已有对照实现）。**归口已完成（`638e96ba`，📋 需求分析）**：① `RESEARCH §2` 增 G16 行（本表唯一非竞品来源）并把真源全集推到 13；② `BASELINE §4` 增 **S11**（内部件持有）；③ `REQUIREMENT §4` 与 `STATE` 议题段同步 13 并标「其中内部件 1 条 · 交付稿可见子集 12」。**设计侧配套的最后一环由本条闭合**：K4 左集改按可见子集现算（D19），否则 4-dev 拿到的是一个必红的闸。本 change 仍不动代码（R7.1 新 change 走），也不把 S11 细节写进交付稿（ADR-001／D11）。
...
| K4 左集改「可见子集」后双向复跑（🧭 交棒项 / D19） | §7.6 (9) 组：真源现算 13、可见子集 12，两份稿各跑新式与旧式 | 合规稿（12 条）新式 diff `0` 绿 ／ 旧式 `2` **假红**；红稿（把内部件条目画进稿，13 条）新式 `2` **抓到** ／ 旧式 `0` **放红** → 旧式两头都错，且消旧式红的唯一省力路径正是 ADR-001/D11 禁止的外发动作，故必须改左集（D18/D13/K8 一族的反向激励第四例）✅ |
| 三套计数不变式（并档后重跑） | `RESEARCH §2` 全集 ／ `REQUIREMENT §4` ／ `STATE` 议题段 | `13 / SAME-REQUIREMENT / SAME-STATE`，可见子集 `12`；顺带发现并修好一处**锚点扫空**：需求侧把 STATE 议题条目抬头拆改了词，登记在 `RESEARCH §2:51` 的 `awk '/出口登记的需代码议题/,…'` 于是扫出 0 行 → 不变式看似 SAME 实为"与空集比"。复原该短语后三套重跑如上（只动 STATE，不碰已投票工件）✅ |
...
# (9) K4 可见子集双向（🧭 交棒项：左集若按真源全集算，合规稿必红、红稿反绿）
R=.specs/product-prototype-refresh/RESEARCH-competitors.md
SLUGS=$(grep -oE '`[a-z]+(-[a-z]+)+`' "$R" | sort -u | grep -v human-approval)
echo "真源全集=$(echo "$SLUGS" | wc -l) 可见子集=$(echo "$SLUGS" | grep -v admin-file-read-jail | wc -l)"   # 期望 13 / 12
echo "$SLUGS" | grep -v admin-file-read-jail | sed -E 's/`([a-z-]+)`/<div data-slug="\1"><\/div>/' > "$S/vis12.html"
echo "$SLUGS"                         | sed -E 's/`([a-z-]+)`/<div data-slug="\1"><\/div>/' > "$S/vis13.html"
K4V(){ diff <(grep -oE '`[a-z]+(-[a-z]+)+`' "$R" | sort -u | grep -v human-approval | grep -v admin-file-read-jail) \
         <(grep -oE 'data-slug="[a-z-]+"' "$1" | sed -E 's/data-slug="([a-z-]+)"/`\1`/' | sort -u) | wc -l; }
K4O(){ diff <(grep -oE '`[a-z]+(-[a-z]+)+`' "$R" | sort -u | grep -v human-approval) \
         <(grep -oE 'data-slug="[a-z-]+"' "$1" | sed -E 's/data-slug="([a-z-]+)"/`\1`/' | sort -u) | wc -l; }
printf '合规稿（只挂可见 12 条）：新式=%s 期望0 ／ 旧式=%s 期望>0 ← 假红\n' "$(K4V "$S/vis12.html")" "$(K4O "$S/vis12.html")"
printf '红稿（内部件条目也画进稿）：新式=%s 期望>0 ← 抓到 ／ 旧式=%s 期望0 ← 放红\n' "$(K4V "$S/vis13.html")" "$(K4O "$S/vis13.html")"
