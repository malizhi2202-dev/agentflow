...
- **action · v10 追加两条（4.2 二审首报 + 主审复核，均 pre-existing、diff 外 → 只进议题包，不开本 change 任务，R7.1）**：⑨ **凭据「使用」面 —— `chat_api.py:18` `POST /agents/{id}/chat` 不校验 agent 归属**：`chat_service.py:48` 裸 id 取行、`:103` decrypt 他人 key 调 provider（拿不到 key 字符串，但能烧配额/冒计费身份）；id 可经 `domain_api.py:144`、`gateway_api.py:39-49` 跨 owner 枚举 → 与 ② 同族，落地时 ②⑨ 一批收口。⑩ 捆绑包：`/scale` `desired_replicas` 无上界 · `delete_domain` 置 NULL 不滤 owner · `models/knowledge_source.py:20` 注释称加密实存明文 · `domain_id=0` falsy 分支绕过 `_assert_domain_access`（自伤级）。
- **verify**: 十条在 `STATE.md` 或 `TOPICS.md` 可 grep 到（⑤⑥⑦⑧ 为 v4 追加 · ⑨⑩ 为 v10 追加）
- 状态: [ ]
