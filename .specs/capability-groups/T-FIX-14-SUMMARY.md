...
### 已知接受 + 理由（🟡）
- **本任务未附带任何测试改动（R4.2 / R4.3 字面要求）**。理由：`write_files` 只有 `AgentBuilder.tsx`，加测试必须**新建**文件 → R6.5 越界、R7.1 扩范围；规则**本体**已被 `capability-group.test.tsx` 14 条钉住（归一那 7 条本轮**逐条数过**，不是转引 STATE：`:51` 空串 / `:57` 纯空白 / `:62` 前后空格 / `:68` 归一后去重 / `:72` 折叠连续空白不折叠大小写 / `:77` 非字符串与非数组 / `:86` 归一后只剩空值），本次未改规则语义，只改「谁调用它」。**已把补齐判据写清交给 5-test**（见「是否触发新工作」），我不越权自建。
