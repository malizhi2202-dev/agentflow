...
- **版本**: **v10.5**（2026-09-24 · v10.1 领域专家 ❌ 落地；v10.2 Master C1/C2 + 安全 ❌ 四改字两登记落地；v10.3 = 🟩 改判 ✅ + (γ) 三分法 + 计数自纠规程加严；**v10.4 = 🔴 改判 ✅ → 4/4 门结，裁决块见文末，两条非阻塞精度（枚举源 `:144` 限定 / `agents_api` 秘钥扫描行 `:45→:61` 内容锚）已落；**v10.5 = T-FIX-14 交接验收轮（5-test 一手复跑通过）：F27 判据可绕性入账（元规则 2 第 5 次同形）· O-16 裁「落」→ T-FIX-15 建卡已派 · O-17 并入 T-FIX-01 且主审加重镜像面 · O-18 进人审清单 · 「单一真相只在前端成立」防误读句入账；**v10.6 = T-FIX-15 交接验收通过（主审八数 + 双证明一手复现）· F27 口径升格「grep 配行为判据」· 主审 4b 自纠 `:151✗→:152` · 队列清零、唯一解锁 = 人签批**）·· 前序 **v10**（回流增补审查）· 历史：**四版四票，每一版都被查出一处取证或判级错误**；v4 最重（F10 方向反了、照它修会造新 bug），v5 修的是**判据本身**（三条 verify 可在缺陷仍存活时为真 / 未修已为真）；v6 收领域专家改票 ✅ 带的 3 条文本级残留；**v7 收 Master 第二轮复核 ❌ 的 3 条（1 条活、2 条已在 v6 修）+ 更正我两次漏读票面 + 登记主审自己截断工件的操作失误**；v9 收「我自己造的假绿」（元规则 4b）；**v10 = 回流增补审查：T-FIX-13 代码首轮进审 + 4.2 二审首次回收 + 两条共享判据的计数重写（含否决 4-dev §6 推断与判死 v9 自己的「13F」假红判据）**
...
| A10 | 服务端请求伪造 | 🟡 命中（**v10.2 改判，原「➖ 不适用」是未审 + 文件归属错** · 安全审计师 ❌ 票首报、主审逐锚复核） | 本面**有**出站 URL 取参，且写侧就在本 change 刚硬化的同族端点上：`agents_api.py:101` 白名单含 `model_config_json`（owner 可写任意 JSON）→ **`services/agent_probe_service.py:31-46` `_resolve_health_url`**（原误记「住在 control_plane_api」——它只是读回口）拼 `{base_url}/health` 或直取 `health_url` → `:53` 后台循环（`interval=3`）→ `:207` `session.get(health_url)` **不带任何 header**（这点反限泄露面：不带走 Agent 自己的 Key）→ 200 时 `:217` `json.dumps(data)` **内网响应体全量入库**、非 200 `:223` `body[:500]` → **`control_plane_api.py:66-69` `AgentProbeLatest` 零 owner 过滤跨 owner 读回给任何认证用户**；唯一缓解 `follow_redirects=False`；脱敏 `:25-27` 只掩 `sk-[A-Za-z0-9]+` → `AKIA…`/JWT/cookie/云 metadata token 直通。链 = 「任意认证用户 → 服务端替其 GET 任意 URL → 响应体读回来」。**全部 pre-existing、本 change 外（R7.1 不改码）**；处置 = **另开 CHANGE 优先修**（STATE 第 5 条口径），锚点随 `T-FIX-12` ⑪ 保藏，并挂进 STATE「进 7-integration 前必须结清」清单 |

#### 本轮新做的扫描（此前既没做也没列进盲区）

| 扫描 | 结果 | 口径 |
|---|---|---|
| 审查面秘钥/凭据模式 | **1 命中，判为非凭据**：`agents_api.py` 的 `api_key = payload.get("api_key","") or "not_set"`（内容锚；行号 v10.4 订正：原文写 `:45`，现居 **`:61`** —— `:45` 是 `if domain_id is not None:`，前轮漂移）是缺失时的**占位默认值**，不是密钥字面量；4 个审查文件内无真实 key/token/AKIA 字面量 | 主审 grep 补做（`-Ei "(api_key\|secret\|token\|passw\|sk-\|AKIA)…[:=]…"`）。⚠️ 顺带一条 🟢：这行把「未提供 api_key」静默吞成 `"not_set"` 再 `encrypt()`，而不是 400 —— 与 F16 无涉，但会让"凭据缺失"在运行期才炸 |
...
**审的是哪段 diff**：v9 的口径锚在 `a28e816f`（v8 tip），它记了下游进度但**没审过 `78117a1e` 的代码** —— 本轮 delta = `acdcda109f00` 线上晚于 v9 锚点的唯一代码提交 `78117a1e`（T-FIX-13：`domain_api.py` +14/−8 · `agents_api.py` +19/0 → 代码面 **+33/−8**，含测试文件 +38/0 整提交 **+71/−8**；**v10.2 勘误：初稿写「+49/−7」不可复现，`git show --numstat` 复核为上述值** —— 数字错但「三文件、零越界、删除行 0」全部复核仍中，Master C1 与安全审计师 🟢 同报，按元规则 4b 形式订正）+ 纯加法测试 `test_capab_scale_allowed_path…` + `c7ca5bcd`（工件采纳，docs-only）。采纳线已由 4-dev 集成进 `acdcda109f00` tip `c7ca5bcd`（STATE 已指路「只读这一条」）。
...
#### 🟡 F23 · R2 Change Propagation：凭据「使用」面**至少三条并列**（v10.2 订数，初稿「还剩一条」少计 2 条）—— ① `POST /api/agents/{id}/chat` 无归属检查 ② 探针链跨 owner 读回 ③ Gemini 进程级全局 Key（**均 pre-existing，非本波回归**）
- **Symptom**：`chat_api.py:18` 不校验 agent 归属，`chat_service.py:48` 裸 id 取行、`:103` decrypt 其 key 调 provider；agent id 的**无门枚举源**（v10.4 精度修正 · 安全改判票 #1，主审实读）= `gateway_api.py:39-49` —— 无任何 owner/域过滤，扫全部 `Domain` + 无域 Agent 并返回 `agent_id`/`agent_name`/proxy_url；原句并列的 `domain_api.py:144` 在 T-FIX-13 后的**非 admin 面已不成立**（该路由自身域门 + `_assert_domain_access` 使他人 Agent 进不了你的域），只作为「admin/旁路面」通道存在 —— 与 A07/O-11 并看，别让半处过度声明给整条 F23 定标打折。任何认证用户可**消耗**他人凭据（配额/计费归因），拿不到 key 字符串本身。
- **Source**：OWASP A01（与 F16 同族不同 sink：F16=取得，F23=使用）。
- **Consequence**：「F16 修完 = 凭据链闭环」如果口径读成「越权者碰不到他人凭据」，这条会让口径说过头。
- **三条并列（v10.2 · 安全审计师 ❌ 票 ② 首报后两条，主审逐锚实读复核）**：① 原文 `chat_api.py:18`/`chat_service.py:48/:103`（跨 owner **消耗**他人凭据，id 可经 `domain_api.py:144`、`gateway_api.py:39-49` 枚举）；② 探针链 —— `agents_api.py:101` 写侧 → `agent_probe_service.py:207` 无 header GET 任意 URL → `:217/:223` 响应体入库 → `control_plane_api.py:66-69` **零 owner 过滤**读回（详见 A10 行 v10.2 改判）；③ `services/llm_providers.py:235` `genai.configure(api_key=…)` 改的是**进程级全局**，`:256` 才实例化模型 → 并发下 Gemini 请求用「最后 configure 进去的 Key」= **跨租户使用凭据，连 agent id 都不需要**，冒计费方向与 ① 相反（你的账烧在我头上）。
- **Remedy（v10.2 按 STATE 第 5 条改处置，安全审计师 ④）**：本 change 内不开任务（R7.1），但处置**不得走「议题包」第三选项** —— STATE 待人工裁定第 5 条对凭据链只开两条合法出路：「本 change 内修」或「**另开 CHANGE 优先修**」（用词「不得缓办」），而 v10 自己的账本行写着「议题包无人读 = 白记」。正解 = **另开 CHANGE 优先修，与 O-11 同批人签（或直接开票）**；`T-FIX-12` ⑨⑪⑫ 只作锚点保藏，不作处置出口。R9.2：① = 4.2 二审（fresh context）首报、②③ = 安全审计师票内首报，主审全部逐行复核锚点后确认（本轮实读 `agent_probe_service.py:25-27/:31-46/:207/:217/:223`、`control_plane_api.py:64-69`、`llm_providers.py:233-258`、`agents_api.py:99-114`）。

#### 🟢 F22 · 工件过时（记给 5-test，本轮不代改）：TEST.md 声明行仍写「安全负例 4 条全 RED」「28 条」，tip `c7ca5bcd` 实际 2绿/1红/1skip、29 条。按元规则 4 单写者，TEST.md 由 5-test 下轮复跑时刷。**Master ③ 追加同族过时（刷时捎带）**：`:54` 既有 41 红的逐文件归因写的是 28+7+3=38≠41，实测口径为 **28+6+4+3**（4-dev SUMMARY 与 v10 两份独立、加总相符）。**[v10.2 更新]** 5-test 复跑时测试基座已至 tip `c7ca5bcd`：29 条 / 定向 16F/12P/1S、安全负例 2绿/1红/1skip —— 一并刷。**同形疤（本工件自己的）**：「修订记录」表的 v7/v9/v8 三行在历轮编辑中混进了「G4 第三轮」代码块内部（现约 `:550-552`）→ 渲染破损、语义未损；v10 行因此不追加进该表，版本沿革以文件头「版本」行 + 本节为准。清扫属 docs 整备，不占判据、不开任务。
...
|---|---|---|---|---|
| **O-14**（F21） | 收口后 UI 扩容副本无凭据 | 三分支（**恢复上游 `TASK:170` 的选项空间**，v10 曾把第二分支换成隐式继承而未说明）：**(α)** 前端扩容入口加「自备 key」（动前端，与 T-FIX-11 同族）· **(β)** 继承「模板自己的 Key」一行式（`domain_api.py` 凭据行 + dev 那条 `not_set` 断言同批改）· **(γ)** 上游原第二分支：**显式的「共享凭据」授权路径 + 落审计**（`credential=` 现仅 `caller-supplied`/`not_set`，`:263`——γ 需要新取值与授权对象，工程量最大） | **(β) 有前置：调用者身份可信 = A07 fail-closed。** 现况 `main.py:204→:210` 不带 `X-User-Id` 即 admin，scale 域门又保留旁路（F24 站点 (a)）→ 匿名者以 admin 身份在 admin 自有域（默认域 `owner_id=admin`，`main.py:143-151`）扩容，(β) 下副本携带 **admin 真 Key**，再经 F23 无归属校验的 chat 路消耗 —— 今天挡住的正是 `not_set`。**注意：O-11 拍 (B) 不满足该前置**（匿名回退拿到的就是 admin 这个用户本身，owner 匹配照过）。身份可信时 **(β)≡(α)**（模板归你 ⇒ 继承=自备），(β) 只在身份不可信时才「省事」，而那恰是它有害的时候（领域专家原话，主审核实）。**[v10.3 三分法修正 · 专家改判票非阻塞建议 #1，主审采纳：原句「(γ) 是唯一不依赖该前置的继承形态」偏强]** —— (γ) 的**授权动作本身也要求身份可信**：A07 fail-open 下匿名者以 admin 身份**自签那份显式授权**再扩容，终态与 (β) 同形，只是多留一行可查的授权记录。诚实读法 = **(α) 不依赖前置（无继承、无授权对象）· (β) 依赖前置且无痕 · (γ) 依赖前置但可查** —— 别让「拍 γ 就能不等 A07」这种读法进人批单 | **弹性缩放事实停用**（不是保持现状）；且 **UI 扩容按钮写死 `desired=5`**（`AgentControlPlane.tsx:703`）+ `current_count>=desired→no_op` ⇒ 第一批 `not_set` 壳造出后同一点击永远返回「已有 5 个副本，无需扩容」——无论拍哪支都需要**删壳重扩/替换语义**的收尾（v10 代价列只写了「停用」半句，此为该半句的下半场）**；**F26（v10.2 登记 · 安全审计师首报，主审实读 `agents_api.py:110-114` 复核）**：`not_set` 约定只覆盖 create/scale 两条路 —— `agents_api.py:112` `if "api_key" in payload and payload["api_key"]:` ⇒ **PUT 传空串 = 静默 no-op、旧 Key 原样留着（掩码照常显示），Key 撤销无 API 路径**（建议判据 `PUT {"api_key":""}` 后 `decrypt(...) == "not_set"`，今天为假）→ **(β) 的代价再加一条：继承来的 Key 除被另一个真 Key 覆盖外关不掉**；`domain_api.py:248` 注释「agents_api 同一个约定」对 update 面不成立，随 O-14 执行批顺路订正 |
| **O-15**（新 · 领域专家 ①，上游 `T-FIX-13-SUMMARY.md` 🟡R6 原文要求入账的半边） | **「域内副本数」一词的所指已被静默换成「调用者在该域内的自有份额」**，四个用户可见面全换了指涉而名字未改：响应键 `current_replicas`（`domain_api.py` no_op 与 scaled 两处返回）、no_op 文案「已有 N 个副本，无需扩容」、前端类型 `stores/domains.ts:28`、`scale_agents` **docstring `:191`（至今写「域内该 capability」「用第一个的配置」——两处收口后都为假：只数你的，`:236` 取 `min(id)`）** | `desired_replicas` 到底约束**「域」**还是**「调用者份额」**（4-dev 已把候选集 `:211` 与计数 `:218-219` 故意拆成两行等人裁：拍「域」→ 正解=**候选按调用者收口、计数按域**，一行改动） | 混属域同一个 `desired_replicas` 现在**多造副本** = 未经签字的可观察行为变更（R6.2 触发人工裁定的把关形态）。**措辞四处随裁定批同批改，裁定前不动**（防语义未定文案先行；docstring 一行勘误可随 02/01 或 O-11 执行提交顺路，属注释非行为） | 用户与外部消费者对「副本数」读到两种互相矛盾的解释；**monitor/告警面按域计数而本端点按份额 → 「desired 已满足」假象 —— 此半句 v10.3 起为代码实据非推导**：`queue_monitor`（`domain_api.py:285`）把 `Agent.domain_id == domain_id` 的**全量域内 Agent（无 owner 过滤）**喂给 `simulate_queue_monitor` → `available_agents`/`needs_scale` 按域计，scale 按份额计，两套计数活在同一个词下（领域专家改判票实证、主审实读 `:285` 复核）；且该端点还是 `T-FIX-12` ②「visibility/越权读跨端点」的又一例 |
| **O-11 站点清单** | v10 补 · 架构师 ✅ 票内追加执行形状（安全审计师 v10.2 票再补第四类旁路） | 若拍 (A′) 保留豁免，改动是 **3 处 / 2 文件**：`domain_api.py` scale 域门（现 `_filter_owner`，政策相反侧）· `scale_agents` 候选集严格谓词 · `agents_api._assert_domain_access`；逐处落审计。**执行形状（架构师 ①③，主审计入）**：站点 (a) **只做 scale 入口的局部替换**（仿候选行写法），**禁改 `_filter_owner` 本体** —— 该 helper 实际 **9 个调用点**（5 读门 + 4 写门，含 `T-FIX-04` 收口的 `:106/:111`），动它 = 连坐全部域端点 | 两处注释「改这一处即可」各自只指自己 —— 以本清单为准（F24）。**顺带勘误（无阻塞）**：`domain_api.py:209` 源码注释称 `_filter_owner`「被 8 个读路径共用」，实为 9 处（口径怎么数都不是 8）；主证据未引用该计数，随 O-11 执行提交顺手改注释即可，不占任务。**⚠️ 与 O-15 同族提示（v10.1）**：O-14 (β) 的前置分析证明域门旁路在 A07 fail-open 下**不只是读问题**——裁 O-11 时把 (a) 站点的取舍与 A07 排期一起看。**第四类旁路（v10.2 · 安全审计师 ③，主审复核属实）**：拿到 `role=admin` 不需要走**任何** role 分支 —— 不带 `X-User-Id` 即 `get_user("admin")`（`main.py:204-210`，`auth.py:185` 同形），今天唯一挡着的是 `main.py:198-200` 的 localhost 判定（CLAUDE.md 自载：反代后失效）；且 `test_api_integration.py:271-274` 把「无头 = 200」写成了断言（主审实跑 `-k missing_user_header`：**今天即红**，TestClient host 不在 allowlist → 403）—— 与本文件 `:616`「先裁 → 换身份 → 再删分支」硬约束同形，谁先动谁背「改坏测试」。**结论：改完上面 3 处 ≠ 收口，A07 与那条断言不同批改完才是**，别让人以为 3 站点清单画完句号就关账 | 漏改 = 口径半开 |
| F23/F25 | pre-existing 面归属 | **v10.2 分治**：F23（使用面三条：chat ① / 探针 ⑪ / genai 全局 Key ⑫）→ **另开 CHANGE 优先修（与 O-11 同批人签），议题包只作锚点不作处置出口** —— 不再适用原「只进议题包」处置；F25 捆绑包四项 → `T-FIX-12` 议题包（②⑨⑩ + ⑪⑫ 锚点） | 若人认为 F23 应升 🔴 本 change 内修，属范围决定（R7.1），说一声即可。**v10.1 升级注记**：O-14 (β) 的害处链**终点在 F23**（无归属校验的 chat 消耗凭据）—— 若人拍 (β)，F23 建议同步升级 | 议题包无人读 = 白记（R18.3 由 STATE 行兜底；v10.2 起 F23 已另挂 STATE 集成前欠项，双保险） |
...
|---|---|---|---|
| 🟫 资深测试工程师（Master） | **✅ 条件通过（C1/C2）** | 09:36:21 | 判定链 ✅ 且升级为结构性论证（`agents_api.py` 不在 02 `write_files` = 互斥，非预估）；A09 案甲 ✅（`:456-492` 钉值行级断言，强于「四件各一条」，两个反空转件）；F22 缓交 ✅（落点在 STATE）。**诚实话入裁决块**：「02→15F」一环**系推导非实测**（02 未落地不可观测），裁决措辞禁止与已实测混写。**C1**：`+49/−7` 不可复现 → 已按 `git show --numstat` 订正（安全审计师同报）。**C2（地雷，本轮已拆）**：T-FIX-02 verify① 打靶含 `agents_api.py:152`/`agent_knowledge_api.py:714` 两站，均不在任务 `write_files` → 守边界者 ① 恒 ≥2、两条出路全被禁止（元规则 1 第三次同形）→ 按他的 (a) 案修：`agent_knowledge_api.py` 补进 02 `write_files`、`:152` 写进 01 action，通过判据 = ① 打靶集与涉事任务 write_files∪action **逐站相等**（已核：7 站全覆盖）。**窗口内解除：02 卡 O-10，不烧实现轮** |
...
| 🟩 领域专家 | **✅**（09:34 ❌ → 两条件落地后**已按自给核验式改判**；复核于 tip `a4c14753` 一手跑：`grep 份额`=3、O-15 命中 6 处，「比点名的还多录一处」） | 09:34:03 ❌ → **09:44:29 ✅** | ❌ 两条件的落地全部通过其复核；**精化半句他认输并记账为他的 R6.2**（(B) 不满足 (β) 前置，锚 `main.py:148-150` 默认域 owner=admin 实证）—— 采强版：(β) 排期硬等 A07。改判票带两条非阻塞：**#1 (γ) 措辞偏强 → 已落 v10.3 三分法**（(α) 不依赖前置 / (β) 依赖且无痕 / (γ) 依赖但可查）；**#2 票面计数时序纠正 → 认，见下方「计数自纠」**。他给 O-15 后果列补了代码实据（`queue_monitor :285` 全量域内喂模拟 → 按域计 vs scale 按份额），主审实读复核属实、已入行。范围蔓延维度背书 ✅（三文件全在 write_files、零命中默认域/未分类、F19/F20 不重开）。他本轮零代码零工件改动 |
| 🔴 安全审计师 | **❌→✅ 改判** | 09:39:42 ❌ → **09:54:03 ✅**（显式改票） | 他先一手复验**码与谱系判 ✅**（重跑 16F/12P/1S 全中、merge-base/零代码/ls-remote 已推为真、`not_set` 诚实性——provider 无 env 兜底→只 401 不偷用平台 Key，**加强**了「取得链闭合」：`:248` 无角色分支连伪造 admin 也拿不到他人明文）。❌ 判的是工件：①A10 行「➖ 不适用」= 未审 + 文件归属错（`_resolve_health_url` 在 `agent_probe_service.py` 非 `control_plane_api`）→ 改判 🟡 A10×A01 五锚全录 + 挂进集成前欠项；②F23「还剩一条」少计 2 条（探针读回链 + `llm_providers.py:235` 进程级 genai Key 后写者胜）→「至少三条并列」；③O-11 缺第四类旁路（无头即 admin、localhost 唯一挡、`test_missing_user_header` 今天即红 403——主审独立复跑确认，**口径已按实跑订为「已经断了没人来收的钉」**，他回「我们说的是同一件事」并采我强版措辞）→ O-11 行补句「3 处 ≠ 收口」；④F23 处置走议题包 = 悄悄换掉 STATE 第 5 条限定过的选项集 → 改「另开 CHANGE 优先修，与 O-11 同批人签」。新登记 🟡 **F26**（Key 撤销无 API：PUT 空串静默 no-op）入 O-14 (β) 代价列 + 🟢 numstat 笔误（与 C1 同源）。**09:54 改票 = 六项逐锚复核全中（他给表），另带两条非阻塞精度 → v10.4 已落**：(a) `domain_api.py:144` 枚举源在 T-FIX-13 后非 admin 面不成立，唯一无门源 = `gateway_api.py:39-49`（主审实读 `:39-48` `Domain` 全量无过滤，属实）；(b) `agents_api.py:45` 漂移实居 `:61`（同文件 `:101/:112` 承重，换内容锚，REVIEW:319 已订）。**他的「覆盖边界」条款原文入账（裁决块引用）**：✅ 只覆盖三份工件在安全维度的完整性可验证性，**不是「安全已可集成」**——引用此票者以 `STATE.md:23` 集成前清单为界（A07 / O-11 / 使用面三条并列）。T-FIX-13 的码维持 ✅，不重开 |

**计数事实（发评前一分钟 `--thread … --tail 30` 重读数出，非复述；本段随 v10.4 定稿再读一次）**：**4/4 ✅**（🟦✅ 09:32:12 · 🟫✅ 条件通过 09:36:21，C1/C2 均已落地不 invalidate · 🟩✅ 改判 09:44:29 · 🔴✅ 改判 09:54:03）。裁决块已出 → 见文末「G4 第五轮裁决」。R2.5/R15.2 期间不变：票面结清 ≠ 可集成。

**计数自纠（第 3、4 次票面漏读，如实入账）**：① 09:42:13 回执写「2/4 · 🟫/🔴 待票」——实际 Master ✅(09:36:21) 与安全 ❌(09:39:42) 均已在我发评前落线，真实现值 2✅/2❌ 四票到齐；② 09:51:16 回执把 🟩 记「待改判」——他 09:44:29 已改判 ✅，早于该评 7 分钟。根因同一：**计数只在 turn 开头读一次，发评时不重读**；且平台「自上次运行以来 N 条新评论」的增量在跨 turn 竞态下少算（Turn D 被告知 1 条，实到 3 条）。**规程加严（入本工件，v7 计数规程的第 4 条修正案）**：任何含票数/「待票」字样的段落，**落笔前一分钟必须重跑 `--thread` 以 author 取最晚票**；turn 内收到过改票消息的，旧读数一律作废。领域专家非阻塞点名（其 #2）收下并致谢——数票是主审职责，不该由投票人替主审数。

### G4 第五轮裁决（v10.4 · 2026-09-24 09:5x · R13.2）

🗳️ G4 审查门 · 第五轮（票面对象：REVIEW.md v10.1–v10.4 + TASK/STATE 同步件，tip `82816bc1`+）
- 🟫 资深测试工程师（Master）: **✅ 条件通过**（C1/C2 → 均已落地 `e6b1e087`；「02→15F」按他的诚实条款在本裁决与工件全程标**推导非实测**）
- 🟦 架构师: **✅**（O-11 执行形状 + `:209` 勘误已入账本）
- 🟩 领域专家: **✅ 改判**（❌→✅ 09:44:29，自给核验式通过；O-14 三分法 (γ) 措辞按其建议 #1 修正）
- 🔴 安全审计师: **✅ 改判**（❌→✅ 09:54:03 显式，六项逐锚复核全中，两条精度随 v10.4 落；他无未决条件）

**结果: 4/4 → ✅ G4 结清，自动进入下一阶段（禁问「要继续吗」）。无异议需记入（两张 ❌ 均以改票收线，条件处置全程留痕于票面表）。**

**覆盖边界（引安全改判票原文防误读）**：本票面**不是「安全已可集成」**。✅ 授权的是 6-review 工件的完整性与可验证性；集成放行另受 **R2.5**（未修 🔴 一项一项修或人签「已知接受」）与 **R15.2**（合并人工）约束，引用任何一张票都以 `STATE.md` 集成前清单为界（A07 / O-11 旁路口径含第四类 / 凭据使用面三条 chat·探针·genai —— 另开 CHANGE 优先修）。
**出口动作**：@集成归档 汇报（产物路径｜自检｜票数｜下一步），队列按 R14 自动推进 = **T-FIX-14 派 4-dev**（独立、无人签阻塞）；`02→01` 继续等 O-10 人选（C2 地雷已拆，人签后可直接派）。

### v10.5 · G4 后的验收轮（T-FIX-14 交付 → 5-test 交接验收 → 主审落账）

谱系：评审 tip `0e8ee534` → 4-dev 交付 `8923c430`（码 `0e30fff2`，`AgentBuilder.tsx` +2/−1）→ 5-test 验收 `8088c527`（TEST/TASK/STATE 三 `.md`）→ 主审 FF 合入后本增量。验收数字主审不转述、全部一手复核：加严 grep 现值 **0**、`capabilitiesOf` 引用 **2**、`AgentBuilder` 测试引用 **0**、`:151` 消费点与 `:164-171` 过滤块实读、T-FIX-00/14 状态行对读一致。

| 编号 | 判定 | 内容与处置 |
|---|---|---|
| **F27** 🟡 | **判据可绕性 = 元规则 2 第 5 次同形（主审自查疤）** | T-FIX-14 的两条【验收】是 `grep -c` 计数，5-test 探针实测 5 种等价内联形状（`?.` / `["capabilities"]` / `|| {}`…）只命中 1 种 → **第 4 套真相可以复活而判据仍为 0**。我和 4-dev 两轮都没查出来，是验收方查出来的 —— 说明「二值」不等于「反同义」。**规程增补（已写进 T-FIX-15 卡）**：grep 计数型判据入 verify 前须附等价形状对抗样本证明。加严版 pattern（§1.9.3，实测现值 0、命中 3/5）已在 T-FIX-15 verify③ 落为新口径；旧已勾任务文本不回改（4d）。 |
| **O-16** → 裁「落」 | 6-review 职权内的修复任务建立，非人拍偏好 | 按 R3.3「主审产出修复任务」建 **T-FIX-15**（前端测试文件写权全仓首次给出；判据/可执行形状直接抄 TEST.md §1.9.4；R14 门后即推 → 已派 5-test）。若人否决该任务 = 直接改任务卡状态，成本一行；O-18 联动条款已写进任务（人拍分门则断言随拍改）。 |
| **O-17** → 并入 T-FIX-01（不新开任务，采 5-test 归类） | 后端 A2 记忆装载吃未归一数组 = **活的载荷 bug，不是「只是没归一」** | ① `[""]` → `if capabilities:` 为真 + `"" in key` 恒真 → **20 条记忆全量注入**（空串归一 = 无能力 = 应为零，此处语义精确取反；前端归一后新造绝迹、存量行还在）；② 非字符串 → `AttributeError` 500；③ **主审加重（5-test 未列）**：无空串列表里不匹配的记忆被**静默丢弃** —— 与 ① 互为镜像，两头无契约。随 01 消费 `capabilities_of` 自然收口；故意不提前钉用例（O-13/FR5 口径）。 |
| **O-18** → 进人工过目清单 | `[""]`/`["   "]` 载荷下**「加载记忆」入口随空 chip 一起消失**（共用 `length > 0` 门，`AgentBuilder.tsx:59`） | 方向 = 向 helper 既有语义靠齐（§1.9.2 八行探针逐行同值），5-test 判**不回归**成立；但按钮与 chip 的**联动**是新的用户可见行为，且随已验收的 T-FIX-14 落地 —— 按 R6.2 精神进人审：接受联动 / 拍「分门」二选一，不判「应该」。T-FIX-15 的断言①显式绑定此裁定。 |
| 防误读句入账 | **「单一真相」本轮后只在前端成立** | 后端 `capability_service.py` **今天不存在**（`ls` 复核），7 处内联一手计数 = 7 → 「第 4 套真相已接上」禁止被读成「统一真相达成」，后端半边卡在 `02→01`（**只等 O-10**，地雷已拆）。已同步 STATE 队列行。 |

**环境事实（第二证人，登记不追根）**：`repo checkout --ref` 报 `exit 129` 但树照常建好落在请求 ref（本轮 `8088c527` 亲验 HEAD 与 origin 同 sha、树干净）。唯一操作纪律：**别把这条报错读成「checkout 失败 → 换 tip 重试」**——那会从错误基线续做。详见 TEST.md §0.1。

### v10.6 · T-FIX-15 交接验收（主审一手复跑，非抄数 —— 学 5-test 两轮的样子）

坐标：`6c458723`（父 `57c3cc56`，边界 `git show --stat` = 1 新测试 96 行 + 3 工件，`+142/−14`，`AgentBuilder.tsx` / `capability-group.test.tsx` / `AgentControlPlane.tsx` / `backend/**` 一字未动 ✓ R3 边界）。**八数全部同值**：targeted `2 passed (2)` · 全量 `7 files / 68 passed / 0 failed` · 加严 grep 现值 **0** · `error TS` 恰 **32**（新文件贡献 0 条）· 后端定向 **16F/12P/1S**、全量对数无扰动（主审本轮重跑与 v10.5 逐字一致）。**双证明一手复现（测量即还原，工作树收尾 clean、`git status` 0 行）**：① revert-and-rerun（页面临时回退 `0e8ee534`）→ `2 failed`，失败原文逐字同（`expected 1 to be +0` / `expected ['a', ' a '] to deeply equal ['a']`）；② **F27 对抗复现**：注入合法等价写法 `var capabilities = (a.model_config_json?.capabilities) || [];` —— 旧字面判据 = **0（绕过，第 4 套真相复活而判据绿）**、加严版 = **1（抓住）**、行为判据 = **2 failed（抓住）**。三层对照一次跑齐，5-test 的 §1.10 表格不是自述，是可复现实验。

**三个交付判断，主审逐条背书**：① 正向锚点（`getByText('wiring-agent')`）堵死「整页没渲染 →『按钮不存在』也成立」的 T5 假绿路 —— 这是对元规则 3 的镜像应用（不存在的断言在什么都不存在时也为真）；② **拒绝为测试向页面要 `data-testid`**（写权外）→ chip 判定收进 `chipTextsOf`/`blankOnlyChipCount` 且不依赖 class/内联样式（T2 配色免疫）：宁可测试写苦一点也不换边界，这是 R6.5 的正确形态；③ O-18 双写绑定（注释 + 工件），并带「改判须连 `:59` 门成对改，单边 = 造第三种状态」的可操作提醒 —— 比我把联动只写进人审清单更防误执行。

**F27 口径升格（采 §1.10 建议，主审不擅自改他卡在文本、升格入本账本）**：grep 计数型判据过对抗样本只是**下限**；规范 = **写法判据必须配行为判据**。本 change 就此完成一次全环：`grep -c`×2（可绕）→ 加严 pattern（钉得更死的写法）→ 行为判据×2（天花板）。今后 6-review 出【验收】时，二值判据旁没有行为判据的，按 F27 同形登记。
