...
## small-model-decisions 域语言（2026-09-22）

**术语**
- **决策模型（System One）**：不生成文本、只输出「带校准概率的类型化决策」的小模型；三型 = Choice（N 选 1）/ Bool（真假概率）/ Score（有序档位分布）。出处 Jev（TypeSafe AI 2026）与社区复刻（Jev-Style-Qwen3.5-2B）。
- **直通路由**：现状的「标签匹配 + 最小连接数」确定性路由。**决策模型未显式绑定时，一切流量走的都是它。**
- **两步制启用**：登记（模型进目录、连通）≠ 启用（按用途逐个绑定：路由决策/对话分诊/简单任务派发）。两个开关独立，全默认关。
- **升级回退（escalated / decision_fallback）**：低置信转大模型 / 决策服务失败转直通路由，两个动作都是审计事件名。
- **微调工厂**：平台编排本机 LLaMA-Factory 用域内对话+记忆数据训练专属决策模型的能力总称；引擎由使用方自装，平台只出任务台账。
- **三指标自测**：准确率 / ECE（校准误差，越小越好）/ 与前一生产版本决策一致率——过门槛才可切生产，门槛在状态转换层校验。
- **简单任务白名单**：服务端枚举的窄任务集（建文件、单句加工类），唯一允许小模型直接产出的通道；枚举外禁止派发。

**已锁决策**
- 显式选择原则：用户不指定/未配置 = 决策模型零介入（AC-0 为铁律，用户原话入账）。
- 微调数据边界：域内隔离 + 域 owner 显式开关 + PII 脱敏 + 可回放台账；默认关，403 引导。
- 硬件基线按消费级：微调 2B LoRA 需显存 ≥8GB 或统一内存 ≥16GB；本地推理 Q4 1.3GB 级不设门槛；不满足=灰显明说，不降级硬撑。
- 评测默认门槛 accuracy ≥80% 且 ECE ≤0.05（对齐 Jev 复刻公开量级），域可改、改动入审计。
- 版本/能力检测用能力探测不用版本号硬判（multica 纪律的同款应用）。

### small-model-decisions · E 层追加（2026-09-22）
- **机器画像**：参谋对本机硬件/后端能力的 ≤5s 探测结果，存本机、默认不上传。
- **环境参谋 / 部署方案库**：画像→匹配→分层部署方案的向导，**只出方案零执行**；方案库是数据（新引擎/新档位加数据不改码）。
- **只出方案**（原"白名单代执行"2026-09-22 被用户撤销）：参谋交付可复制命令/可下载脚本/操作路径，不装不下不写配置；连通状态靠只读再探测刷新。
- **已锁决策**：画像测不出 = 只推云 API 档+手动方案并明写缺因，禁止猜测凑方案；机器画像默认不上传（用户确认）；参谋零执行。
